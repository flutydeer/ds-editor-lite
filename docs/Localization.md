...
- 需要裁剪小数尾零时读取 `QLocale().decimalPoint()`，不要写死 `.`；
...
音名、API level、version 等稳定标识符继续使用明确规定的 C Locale 或协议格式。保留这类
例外时应在代码附近说明原因，避免以后误迁移为用户显示格式。
...
- 用户界面整数不显示组分隔符，C Locale 例外仅限机器文本和稳定标识符且有注释；
