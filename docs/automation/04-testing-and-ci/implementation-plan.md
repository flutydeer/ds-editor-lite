...
7. 结合 Linux CI 覆盖率及本地、专项采样确认重要遗漏，优先补填词、导入、设置、声线与音素等普通 GUI 工作流，以及批量编辑、真实应用适配和推理状态变化；按逻辑覆盖方向目标补齐行为，不靠分支穷举或无语义断言提高数字。GAME/RMVPE 提取模型本期不扩展。
...
后续补测仍进入既有套件：应用 GUI 验证曲线 Shape/Scale、内联文字、音素拖动和波形过期结果；公共控件验证预览与提交；协议层验证批量路由、导入计划复验及填词，实际音频批量提交由应用工作流验证。NativeDesktop 承接钢琴窗及轨道的 Null RHI 输入、帧提交和条件设备用例；设备发现与显式配置规则见[执行计划](test-plan.md#3-运行条件)。

继续补齐同一职责中的正常流程：实际读音/音素任务结果在编辑期间暂存、结束后应用或随对象换代丢弃；主窗口面板、分离与嵌入设置、日志和规则编辑验证用户操作。保存决策复用生产文档状态机与保存器，仅替换外部 UI 回答，按文件工作流记账；公共参数查询验证范围与点数预算，不复述字段声明。
