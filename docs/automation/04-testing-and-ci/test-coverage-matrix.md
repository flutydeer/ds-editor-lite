...
| 包刷新提交被拒绝 | workflow | 实际包扫描得到新候选目录，但提交条件不再满足时保留已发布包、歌手查找及通知状态；恢复原搜索路径后正常刷新 | ApplicationWorkflows::rejectedPackageRefreshKeepsThePublishedCatalog | 默认内置声库；隔离目录 |
