...
| 损坏声学模型的加载与恢复 | workflow/process | 临时副本中的声学模型损坏时，实际 Editor 推理进入失败终态且不发布音频缓存；修复模型后同一进程可重试并导出有效 PCM | ModelResources::voicebankInferenceAndWaveExport 的 repaired-acoustic-model 数据行 | 始终使用内置微型声库副本；不修改显式配置的真实声库 |
