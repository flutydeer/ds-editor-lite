...
| 公开保存的隐式路径、授权和替换准入 | workflow/protocol | 无路径保存拒绝且保留文档；另存为后修改工程，拒绝策略阻止替换；收紧目录授权时隐式保存不得覆盖磁盘，恢复授权后保存成功且不增加撤销，随后允许打开新工程 | ApplicationWorkflows::publicSaveChecksTheCurrentPathBeforeReplacingTheDocument | 通用；临时 DSPX；实际文件授权与保存器 |
