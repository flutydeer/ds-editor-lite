...
| 实验 RHI 编辑后端 | gui | 几何与字形不能替代真实控件；实际析构暴露私有状态释放后的事件重入 | Null 后端检查音符和片段手势、连续擦除、内联文字及菜单；原生捏合缩放保持输入点对应时间，随后时间尺和键盘滚轮仍联动缩放；绘制工具与选择工具双击新建共用预览、提交和历史断言；音符左右裁边共用数据行，撤销绘制后按历史范围定位已不存在的音符；音高绘制/描摹/擦除、锚点追加预览和取消检查模型及插值保留。播放跟随在拖动中暂停、结束后恢复；验证单次提交、命中、撤销和帧，保留正常析构 | EditorRendering、NativeDesktop 的 tst_rhi_editor.cpp、tst_rhi_tracks.cpp | 原生窗口；Linux Xvfb；无需物理 GPU；不验证像素 |
