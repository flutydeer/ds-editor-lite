...
| 钢琴窗、轨道编辑、快捷键和视口 | gui/domain | 既有几何/事件回归，部分仅测算法；首次采样显示已有音符交互不足 | 已补实际钢琴窗绘制/提交/撤销；现补已有音符拖动提交与 Escape 取消。快捷键建立真实可见 owner 与焦点；视口纯逻辑分离 | PianoRollGuiIntegration、PianoRollInteractions、PianoRollNoteCommit、TrackEditorInteractions、EditorShortcuts、EditorViewController、EditorViewportController、EdgeAutoScroll、UndoRedoController | 通用/offscreen/原生 |
