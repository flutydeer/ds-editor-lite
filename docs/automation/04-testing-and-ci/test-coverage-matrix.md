...
| 歌词、音节、本地化、关联音素属性 | unit/domain/gui | 已有排序及属性级联回归；首次采样中 TextSplitter 未执行 | FillLyricTaggerOrder 归并改名为 LyricRules；增加混合文字保真、规则启用/优先级及空匹配回归，修复空匹配重复文字；新增部分 Windows 定向通过，Linux 待验证 | Syllabification、LyricRules、WordPropertyCascade、LocalizedText、UiLanguage | 通用/offscreen |
...
| 整片段复制、跨轨粘贴与参数保真 | domain/gui | 首次采样中 ClipsInfo 未执行；序列化遗漏片段参数 | 复用音符剪贴板曲线编码，补全部参数层、无音符曲线、声线/发音保真和跨轨批量撤销；生产控制器复用同一粘贴准备逻辑；补真实 GUI 入口 | NoteTransfer、PianoRollGuiIntegration | 通用/offscreen；新增部分 Windows 定向通过，Linux 待验证 |
...
| GUI Host 无设备时的公开播放失败 | process/gui | Linux 实际 MCP 调用超时；无设备时错误弹窗阻塞调用 | 仅 TrustedGui 调用显示设备错误弹窗；公开调用检查失败、状态不变及后续查询；GUI fixture 关闭自身设备确定性验证无弹窗 | McpProcessIntegration、PianoRollGuiIntegration | offscreen；Windows 定向通过，Linux 待验证 |
...
| 推理结果与当前文档、输入及编辑会话匹配 | workflow | 首次采样中 InferenceApplyGate 未执行；输入转换测试不能代替完成门控 | 新增真实任务快照到门控的 Apply/Drop/Defer 行为，覆盖四阶段输入变化、文档/对象消失、无关 revision 变化和编辑冲突；Windows 定向通过，Linux 待验证 | InferenceWorkflow | 通用；无需模型输出 |
...
| Connector 生命周期与 stdio | protocol/process | 长入口及手工子集分派；Linux 揭示 stdio 测试硬编码 .exe | 拆可定位用例，保留真实流行为；可执行路径改由生产 CMake target 提供，Windows 定向通过，Linux 待验证 | DsConnectorLite | 通用 |
| Editor 启动、服务、单实例和退出 | process | Windows 数据根假设；Headless 混入 GUI 场景；Linux 替代进程未就绪且存活/所有权/清理实现缺失 | 跨平台沙箱，共用进程设施，分运行条件；重启输出改为沙箱文件，补跨平台替代进程管理，Linux 待验证 | HeadlessProcessIntegration、McpProcessIntegration、SingleInstance | 通用/GUI/平台 |
| 钢琴窗、轨道编辑、快捷键和视口 | gui/domain | 既有几何/事件回归，部分仅测算法；首次采样显示已有音符交互不足 | 已补实际钢琴窗绘制/提交/撤销；现补已有音符拖动提交与 Escape 取消。快捷键建立真实可见 owner 与焦点；视口纯逻辑分离 | PianoRollGuiIntegration、PianoRollInteractions、PianoRollNoteCommit、TrackEditorInteractions、EditorShortcuts、EditorViewController、EditorViewportController、EdgeAutoScroll、UndoRedoController | 通用/offscreen/原生；新增部分 Windows 定向通过，Linux 待验证 |
...
首轮实际采样来自 PR head `59aa9cd0` 的 [Actions 34278449245](https://github.com/flutydeer/ds-editor-lite/actions/runs/34278449245)：行覆盖 46.9%，分支覆盖 38.3%，该轮 67 项中 3 项失败，不能作为最终验收结果。完整口径、分母、目录汇总与失败项见[测试报告](test-report.md)。

据此优先补整片段剪贴板、推理完成门控、歌词拆分和已有音符真实拖动/取消。新增实现的结果须在后续候选重新采样；不把当前代码存在等同于已覆盖。原生渲染、声库/设备及主观观感按运行条件记录，不为降低未执行行数扩展像素基线或模型矩阵。
