...
| 主窗口分隔条拖动与面板恢复 | gui | 经真实手柄拖动调整比例，分别折叠轨道和底部面板；检查拖动状态同步、重新显示后的比例和当前页保留，文档及历史不变 | NativeDesktop::mainWindowSplitterDragRestoresPanelSizes | 原生桌面；Linux Xvfb；同一窗口内比较还原尺寸，无截图基线 |
