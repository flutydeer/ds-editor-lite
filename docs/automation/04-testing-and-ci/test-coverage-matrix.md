...
工程不变和撤销恢复的内容快照共用正式 DSPX 编码，包含片段、音符、参数及声线，不使用遗漏片段的旧模型/轨道序列化作为完整内容依据。对象身份、版本、历史及非持久化状态仍由所在用例独立检查；保存格式的校验和文件发布另由文档工作流验证。
...
| 参数曲线与轨道片段的真实编辑接线 | gui | 数值和 Facade 用例未进入 CommonParamEditorView 与 TracksGraphicsView 的实际鼠标提交路径 | 补参数绘制、Shape/Scale 和 Legacy 音高调制，后者以真实推理基音验证选区结果；检查预览、范围外保留、一次提交、取消及撤销；点击选区外后可重新框选，不沿用旧变换。轨道视图检查片段选择与拖动 | ApplicationGui 的 tst_parameter_editing.cpp、tst_pitch_modulation.cpp、tst_track_editing.cpp | offscreen；音高调制默认内置声库；无需设备 |
...
| 应用运行状态、设置、播放、包与规则 | domain/workflow | RuntimeDomains/L3ApplicationDomains 按历史阶段分开，设置及包 fixture 重复，歌词替身复制排序逻辑 | 合并到 ApplicationServices；共用 Harness，规则使用明确 Host 快照和实际持久化断言。稀疏设置更新检查推理开关、预看和音高平滑且保留其他设置；Tagger 同时更改名称、语言及条目时保持身份并更新语言排序键 | ApplicationServices、Preferences | 通用 |
...
| 文档输入失败后的恢复 | workflow | 公开文件准入不能证明 GUI 工作流的错误状态已释放 | 打开/导入缺失或不支持的文件先报告错误，保留未保存工程与历史，随后可有效打开；取消新建后保存未命名工程，取消路径选择不改变状态，再次保存正确标记保存点 | ApplicationGui::rejectedProjectInputAllowsTheNextRequest、newDocumentHonorsTheSaveDecision | offscreen 程序承载；提示回答替身；真实文件和生产状态机 |
...
| 音频导出配置、进度与关闭 | gui | 实际文件导出不能替代配置及进度对话框的事件接线验证 | 通过真实输入检查格式/采样率与预览、轨道选择/混音与文件计划、取消及重开恢复；文件选择器取消保留配置，选定目录与格式后更新混音及分轨文件名模板；复用小型 WAV，点击 Export 经生产导出器检查完成提示、进度达到 100%、实际削波警告及列表展开/收起、Close 关闭、任务清理和进度窗口释放 | ApplicationGui::exportFormatUpdatesFileNamePreview、exportSourcesAndMixingUpdateFilePlan、canceledExportConfigurationDoesNotPersist、audioExportProgressCompletesAndCloses | offscreen；无需声库/设备 |
...
| HTTP 在途请求遇到路由关闭或服务停止 | protocol | 正常请求完成后的启停不能证明等待中响应与准入释放 | 受控处理线程覆盖 MCP/Native 等待中停服、仅禁用 MCP 时保留 Native 请求，以及重新启用后的正常调用；晚到处理结果不能覆盖关闭响应 | AutomationProtocol::pendingRequestsRespectRouteAndServerShutdown | 通用；本地回环；信号量控制处理时机 |
| 推理设置的设备发现和稍后重启 | gui | 采样参数持久化未触达异步设备发现与 GPU 选择 | 真实下拉框选择 Provider，设备可用时保存具体设备并恢复默认，无可用设备时回退 CPU；选择稍后重启保留有效运行时、工程及历史 | ApplicationGui::inferenceProviderSelectionDetectsDevicesAndDefersRestart | offscreen；按当前系统实际设备发现结果执行；无需 GPU 推理 |
...
| 公开音频导出的来源、预览与内容 | workflow/protocol | 直接业务调用未经过轨道 ID、混音设置转换和公开文件计划 | 既有 WAV/FLAC 场景经公开接口选择轨道、预览并实际导出；公开独奏、轨道/片段静音、轨道/片段/主输出增益及立体声声像实际改变采样内容，保留削波警告和完整撤销断言；分轨输出拒绝重名文件，修正模板后检查各轨独立 PCM；导出用例共用终态等待，素材关闭后清理临时目录 | ApplicationWorkflows::audioExportRespectsRangeMixAndMute、separatedAudioExportRejectsCollisionsAndKeepsTrackSignals | 通用；小 WAV；无需设备 |
...
| DSPX 编辑内容往返 | workflow | 原文件测试多检查空工程、JSON 头或对象存在，未核对编辑内容 | 补真实 save/load 的乐句、发音、参数、声线混合、tempo/meter 保真；轨道动态声线验证缺失声线后的关键帧比例重映射、身份保留及再次保存 | DocumentIO | 通用 |
...
| 音频资产解析、来源换代和解码通知 | workflow | 路径/哈希与解码控制分散，解码测试替写 AudioContext、DocumentWorkflowController 等生产方法；公开路径更新缺少实际准备与提交接线 | 合并 AudioAssets 并复用真实运行时；公开确认/重定位经 Registry 和 Host 准备，检查路径、格式、哈希、真实解码、单次提交及撤销，公开任务查询可见，损坏文件、提交前取消、准备期间工程编辑或撤销文件授权均不提交旧结果，恢复条件后可重试 | AudioAssets、ApplicationWorkflows::publicAudioPathUpdatesPrepareCommitAndUndo | 通用；小型 WAV；无需播放设备 |
...
| 模型推理完成时的编辑等待与恢复 | workflow | 四阶段门控判定不能证明实际流水线进入等待、解除等待和应用结果的接线 | 暂停真实 duration/pitch/variance/acoustic worker 后开始编辑，完成时保留原结果及文档/历史，结束编辑后恢复流水线并应用输出；声学结果等待期间换代文档，旧结果不能进入新工程 | ApplicationWorkflows::modelInferenceWaitsForEditingBeforeApplying | 默认内置声库；CPU；受控 worker；无需播放设备 |
...
| 公开推理状态与任务作用域关联 | workflow/protocol | 完整模型输出不能证明状态查询关联了正确的公开任务；原范围字符串比较不能关联轨道任务与片段查询 | 用实际能力查询返回的后端和模型发起轨道推理；无效轨道、不可用的后端/设备/模型及不连续的下游阶段请求均保留已有结果、文档和历史，不启动 worker。暂停真实 Pitch worker 后检查轨道及片段查询的任务关联，完成阶段及其他轨道不误报；成功后清除关联，重置目标音高使下游失效并能再次推理，上游及其他片段保留 | ApplicationWorkflows::publicInferenceStatusAssociatesTasksWithTheirScope | 默认内置声库；CPU；目标轨道含两个片段；无需播放设备 |
...
| Connector 分页缓存与离线调用 | protocol | 缓存与本地调用混入无 SLA 的循环次数和速度门槛 | 握手消费上游分页，桥接列表沿游标遍历无重项且不额外请求上游；快照刷新拒绝旧游标，重新遍历保留有效工具并移除失效工具；离线调用检查实际状态，保留协议自身截止时间 | Connector::paginatedHandshake；既有缓存与离线调用用例 | 通用；受控编辑器服务；不约束产品工具总数 |
...
| 钢琴窗、轨道编辑、快捷键和视口 | gui/domain | 既有几何/事件回归，部分仅测算法；首次采样显示已有音符交互不足；编辑视图与撤销控制分散 | 补实际钢琴窗绘制/提交/撤销、已有音符拖动提交与 Escape 取消；快捷键建立真实可见 owner 与焦点；轨道头、速度栏和拍号栏的滚轮检查两轴导航、轨道头行高与画布滚动同步且不修改工程；控制器、视口、输入与滚动统一归入 EditorInteraction，生产应用完整接线留在 ApplicationGui | ApplicationGui、EditorInteraction、ProjectEditing | 通用/offscreen/原生 |
...
| 实验 RHI 编辑后端 | gui | 几何与字形不能替代真实控件；实际析构暴露私有状态释放后的事件重入 | Null 后端检查音符和片段手势、连续擦除、内联文字及菜单；原生捏合缩放保持输入点对应时间，随后时间尺和键盘滚轮仍联动缩放；绘制工具与选择工具双击新建共用预览、提交和历史断言；音符左右裁边共用数据行，撤销绘制后按历史范围定位已不存在的音符；音高绘制/描摹/擦除、锚点追加及曲线合并预览不修改工程，合并保留节点身份和内容，取消及撤销恢复。播放跟随在拖动中暂停、结束后恢复；验证单次提交、命中、撤销和帧，保留正常析构 | EditorRendering、NativeDesktop 的 tst_rhi_editor.cpp、tst_rhi_tracks.cpp | 原生窗口；Linux Xvfb；无需物理 GPU；不验证像素 |
| 输出设备配置与实际播放 | workflow/gui | 受控回调和无设备失败不能替代真实输出设备路径 | 自动探测或按名称指定设备，通过真实设置页选择默认设备别名、缓冲及采样率，检查实际设备、落盘配置及工程不变；静音素材验证公开播放/暂停/停止和回调推进，恢复自身配置 | NativeDesktop::availableAudioDeviceRunsPublicPlayback | 无可用设备明确 QSKIP；已枚举或指定设备的失败为 FAIL；设备不提供可选缓冲或采样率时检查控件禁用 |
...
| 缓存复用后的清理与文档换代 | workflow | 仅检查登记文件不能证明当前工程通过缓存探测恢复的音频仍受保护 | 实际推理后以同一乐句重新建立文档，复用声学结果但不登记新声学任务；清理保留当前 WAV 及输入内容，删除无引用缓存；换成空文档后释放旧缓存，保留无关用户文件 | ApplicationWorkflows::cacheCleanupProtectsRestoredInference | 内置声库；CPU；临时缓存；无需播放设备 |
...
| 手工音素与动态声线的公开读回 | protocol | 领域测试未验证音符草稿和动态快照的公开转换 | 音符插入保留无时序或完整时序，部分偏移拒绝整批；动态声线读回完整权重及关键帧身份，移动后可撤销，旁路保留关键帧，查询不改文档和历史 | AutomationProtocol::insertedPhonemesPreserveTimingAndRejectPartialOffsets、routing 的 voiceAndSpeakerMix | 通用；所属领域继续负责详细业务矩阵 |
| 音素边界与发音候选菜单 | gui | 普通拖动未覆盖边界限制，发音区域缺少真实菜单操作 | 在同一乐句上验证相邻音素、下一音符和末尾限制及撤销；修改速度后已有视图使用更新后的相邻边界；点击候选只修改目标音符 | ApplicationGui::phonemeBoundaryDragCommitsAndUndoRestoresOffsets、pronunciationMenuChangesOnlyTheClickedNote | offscreen；音素边界使用声库 |
...
| 实际批量文件导入 | workflow/gui | MIDI/DSPX 经公开预检和批量入口调用真实加载器，复用场景检查仅验证、排队取消后重试、全部有效/混合失败/全部失败、原子回滚及允许部分成功；受理后源文件变化时拒绝旧计划，原子模式不提交前项，尽力模式只提交有效项；加载期间编辑工程导致提交冲突，保留新编辑和历史；轨道视口的真实拖放经 DocumentImportController 导入两份 WAV，检查预览无修改、一次提交、撤销重做和视图恢复 | ApplicationWorkflows::projectBatchImportUsesRealLoaders、ApplicationGui::droppingAudioFilesCommitsOneBatchToTheSelectedTracks | 通用/offscreen；临时文件 |
...
| 预设和歌词规则的生产持久化 | workflow | 旧服务替身不能验证 AppOptions Adapter/Store；补创建、更新、重开、删除及规则的实际语言结果；规则写盘失败时配置、规则列表和运行时分词语言均保持原值，恢复写入后重试生效，文档与撤销历史不变 | ApplicationWorkflows::speakerMixPresetPersistsThroughTheProductionStore、lyricRulesUseTheProductionRuntimeAndPersistence | 隔离配置 |
...
| 公开主题和界面语言更新 | protocol/gui | 公开请求经过实际 AppOptions 适配器更新已打开主窗口；配置文件暂不可写时，设置、主题/翻译和文档历史共同保留，恢复后同一请求成功并落盘；检查时间轴颜色、菜单译文及快捷键 | ApplicationGui::publicUiSettingsPersistAndRollback | offscreen；真实主题、翻译和隔离配置；复用主窗口 fixture |
...
| 填词预览、编辑与规则 | gui | 实际声库参与歌词转换，经过真实控件拆分、修改预览、导入音符、取消及撤销；LRC 文件选择取消保留输入，实际导入移除时间戳，收起预览后从文本提交，选项同步保存；规则编辑验证启用及删除前后的实际预览、Apply 提交和持久化，删除自定义规则保留内置规则 | ApplicationGui 的 tst_fill_lyric.cpp | offscreen；默认内置声库；临时 LRC |
...
| 歌词搜索模式及正则匹配 | domain/gui | 界面和接口维护两套匹配，正则可选分支的完全匹配分别出现漏报和误报；统一使用业务查询，在表达式编译时约束匹配范围，复用领域数据行及实际搜索窗口检查模式、大小写、无效表达式恢复和结果定位 | ProjectEditing::noteSearch、ApplicationGui::lyricSearchNavigatesTheActualEditorAndHandlesNoMatches | 通用/offscreen；查询不改变工程与历史 |
...
| 音素、搜索、动态声线与编辑边界 | gui | 真实对话框和输入验证音素确认/取消/重置、边界拖动及撤销；音素列表经行按钮和右键菜单增删，检查新行继承语言、改变语言后提交及取消草稿保留原值。波形加载使用实际推理音频，受控延迟检查切换片段后丢弃过期结果。内联歌词/读音检查双击、提交/取消和导航；编辑期间删除目标音符会关闭编辑器，撤销恢复原文且不提交暂存文字；缩放后省略的歌词悬停可读回完整文字，离开后提示隐藏且不修改文档，保留搜索、动态关键帧、音符缩放及时间尺循环行为 | ApplicationGui 的 tst_note_dialogs.cpp、tst_dynamic_mix.cpp、tst_piano_roll.cpp、tst_track_editing.cpp | offscreen；音素和波形使用内置声库 |
| 歌词整体后移与钢琴键盘 | gui/domain | 实际菜单检查连续选区变连音、文字属性后移、手工音素清理及撤销，非连续选择禁用；键盘滑奏和隐藏检查 note-on/off；完整钢琴窗改变音区后点击相同位置得到对应音高，键盘滚轮实际移动编辑器视口 | ApplicationGui::movingLyricsBackwardUsesTheSelectedWordRange、pianoKeyboardGlissandoAndHideReleasePressedNotes、pianoKeyboardRangeAndScrollingFollowTheEditor | offscreen；无需音频或 MIDI 硬件 |
...
| 钢琴窗锚点创建、框选与合并 | gui | 完整 Graphics View 编辑器验证单锚点与连线预览尚未提交、Escape 取消、补足曲线后提交；框选成组拖动、插值/删除菜单、跨曲线连接和节点转移均检查模型、预览绘制变化、取消及一次撤销；转移保留节点身份 | ApplicationGui::pitchAnchorCreationPreviewsBeforeCommitting、pitchAnchorRangeEditsUseTheViewAndMenu、pitchAnchorMergePreviewCommitsAndUndoes | offscreen；真实控件事件；无声库和设备要求 |
...
| 声音导出等待推理时取消或换代 | workflow | 复用实际声库准备过程，分别取消导出和换成同内容的新文档；旧文件不被覆盖且不开始渲染，换代后旧任务不可再查询，普通取消保留原历史。后台任务排空后当前文档再次导出成功，文件可解码、样本有限且非零，无暂存残留 | ApplicationWorkflows::cancelingVoiceExportDuringPreparationAllowsAnotherExport | 默认内置声库；独立缓存；无需音频设备 |
| 公共曲线编辑与音素名称转换 | protocol | 混合绘制/锚点曲线经公开输入保留位置、步长、值和插值；使用创建返回的曲线身份继续批量插入和移动锚点，保留其他曲线，撤销恢复完整原模型。音素名称按有效语言补全并清除失效偏移；公开重置保留原始音素和时间，重复重置不增加版本或历史，两次撤销依次恢复编辑和初始工程 | AutomationProtocol::publicParameterEditsPreserveCurvesAndUndo、phonemeNamesUseTheEffectiveLanguageAndResetOffsets | 通用；真实领域实现；无需声库 |
...
| 菜单导入、MIDI 编码和通道重解析 | gui/workflow | MIDI/DSPX 从实际菜单和 Qt 文件选择框进入配置页，选择后取消不修改文档，确认后导入所选轨道；错误编码与 UTF-8 切换更新歌词预览并保留选择；同一 MIDI 轨中的不同通道可合并或分离，重解析后的选择对应最终导入，音符时间和音高保留，单次撤销恢复原轨道 | ApplicationGui::interactiveProjectImportRespectsSelectionAndCancellation、midiChannelSelectionRebuildsTracksBeforeImport | offscreen；小型 MIDI/DSPX；真实菜单、文件选择、配置页及加载会话 |
...
| 参数变换边界与倍率手柄 | gui | 收拢过渡区使边界重叠，再按拖动方向重新展开；拖动过渡区及核心区边界，保持未提交模型和已有过渡宽度；倍率手柄改变核心值，过渡区平滑衔接且范围外保持，预览图形更新，单次提交可撤销恢复 | ApplicationGui::parameterTransformHandlesControlTheTransitionRange | offscreen；实际参数控件及输入；无需声库 |
| 文件菜单的打开、保存及导出 | gui/workflow | 真实 Qt 文件选择框处理 Unicode 路径；取消打开不换文档，取消另存为保留路径和未保存历史，确认后更新保存点/最近列表，后续 Save 使用新路径；独立重读磁盘验证新文件更新且原文件不被覆盖。导出子菜单经取消/确认实际生成 MIDI 并解析音符，打开音频导出配置后取消，保留工程、保存点及历史 | ApplicationGui::fileMenuOpensSavesAndExportsThroughThePicker | offscreen；临时 DSPX/MIDI；Qt 文件选择框 |
...
| 公共单文件加载与计划接线 | workflow/protocol | DSPX、MIDI 和外部格式共用工作流，经格式计划、Registry 和实际 Host 追加或打开；追加保留已有轨道且可撤销，打开验证工程身份与保存状态；MIDI 保留绝对音符时间、歌词及时间线，并作为未保存文档打开。受理后取消、撤销源文件授权、替换源文件或编辑当前工程时，拒绝旧任务提交并保留当前状态 | ApplicationWorkflows::publicProjectLoadUsesThePreparedPlan | 通用；临时工程；外部转换进程替身；实际加载器 |
| 公开保存的隐式路径、授权和替换准入 | workflow/protocol | 无路径保存拒绝且保留文档；另存为补全或替换扩展名，验证模式不写文件、不绑定路径，实际保存只写最终 DSPX 且保留原名文件；修改工程后拒绝策略阻止替换；收紧目录授权时隐式保存不得覆盖磁盘，恢复授权后保存成功且不增加撤销，随后允许打开新工程 | ApplicationWorkflows::publicSaveChecksTheCurrentPathBeforeReplacingTheDocument | 通用；临时 DSPX；实际文件授权与保存器 |
| 公共声线预设与已应用混合的生命周期 | workflow/protocol | 通过实际声库解析公开预设的保存、查询、同名更新和应用；公开混合查询保留预设来源身份、名称和未修改状态，读取不改变工程历史，撤销清除来源；删除预设后，已应用混合保留，失效引用返回错误且不改变工程或历史，撤销恢复原声线 | ApplicationWorkflows::publicSpeakerMixPresetsResolveAndPreserveAppliedVoices | 默认内置声库；至少两条声线；隔离配置 |
...
| 片段复制、跨轨移动与裁边 | domain/protocol | 混合片段复制保留时间、声音、文字及参数，新对象使用独立身份，重试不重复创建，一次撤销/重做恢复整批；公开场景使用创建返回的片段身份继续跨轨移动和左右裁边，保留源乐句、音符内容及撤销边界 | ProjectEditing::duplicateClipsPreserveContentAndCreateIndependentObjects；AutomationProtocol::publicClipCopyAndMovePreserveTheSourcePhrase | 通用；真实模型；无需文件解码、模型或设备 |
| 混音通道编辑与电平反馈 | gui | 轨道及主输出的文本输入检查提交、取消及撤销；轨道换序及撤销重做保持通道顺序、编号和电平表绑定，原控件继续编辑原轨道；电平更新同步峰值文字，点击削波指示只清除所选通道的提示，不修改工程或历史。电平组件另以短保持时间检查峰值衰减、新峰值中断衰减及削波提示保留 | ApplicationGui::mixerChannelInputsAndLevelsStayScoped；GuiComponents::meterPeaksHoldDecayAndKeepClippingLatched、aNewMeterPeakInterruptsDecay | offscreen；实际 MixConsoleView；无需音频设备 |
...
| 参数栏的前景/背景切换 | gui | 实际下拉选择和交换按钮同步视图及显示文字；可见范围内的真实曲线在交换或清除背景时更新画布；清除背景后交换不改变前景，操作不修改工程或历史 | ApplicationGui::parameterToolbarSwapsTheVisiblePairWithoutEditingTheDocument | offscreen；无需声库；同次运行画面变化，无截图基线 |
