...
- 参数 Shape/Scale 经真实选区和拖动验证预览、范围外保留、一次提交、取消及撤销重做；内联歌词和读音经双击、键盘提交/取消及音符间导航验证模型与编辑状态。
...
默认钢琴窗及轨道编辑器使用 Legacy/QGraphicsView；实验 RHI 后端需在开发者设置中显式启用并重启。`NativeDesktop` 使用真实 `PianoRollRhiWidget`、`TracksRhiWidget` 和 Qt 的 Null 后端，验证音符绘制、片段跨轨拖动及裁边的预览、提交/取消、撤销重做、实际命中和帧提交。Null 执行 CPU 渲染准备和命令路径，不验证 GPU shader 的像素结果；不建立设备或截图矩阵。Qt offscreen 不支持 RHI，该用例使用原生窗口环境。
