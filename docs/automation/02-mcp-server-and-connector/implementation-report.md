...
Automation 设置已进入选项对话框导航，并使用与其他选项页一致的主题图标。页面、运行状态和 24 个域的显示文本已补齐中文翻译，其中 `history` 显示为“历史记录”。控制层级、自定义工具集和文件访问权限合并到“访问控制”小标题下的同一个连续卡片；自定义工具集条目显示 Custom 已启用数/总数，并提供导入控制层级按钮和进入二级页面的箭头。工具组卡片在二级页面首次进入时才创建，并按公共契约的领域排列；固有的 L0 工具不进入 Custom 列表。每组默认收起，支持独立展开/收起、启用计数和整组启停。导入操作可以将当前所选 L1、L2 或 L3 的完整工具范围写入 Custom；选择 Custom 时按钮禁用。单工具开关仍直接写入同一 `customPermissions` 集合，组级操作与预设导入只更新这套既有权限，不引入第二套配置。
