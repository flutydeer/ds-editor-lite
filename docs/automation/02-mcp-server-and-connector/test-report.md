...
同一候选连续三轮完整 CTest 均为 67/67，通过合计 **201/201**，0 失败、0 超时。GUI/Computer Use 形成 68 份界面证据，最终成功路径未出现模态弹窗、界面假死或无人值守阻塞。测试中发现的缺陷均已修复并重跑受影响域及三轮门禁，最终判定为 **PASS**。
...
| 最终测试清单 | 67 项 CTest |
| 长音频提取修复定向回归 | 6/6 通过，0 失败，17.36 秒 |
| 完整 CTest 第 1 轮 | 67/67 通过，0 失败，92.44 秒 |
| 完整 CTest 第 2 轮 | 67/67 通过，0 失败，92.36 秒 |
| 完整 CTest 第 3 轮 | 67/67 通过，0 失败，92.25 秒 |
| 三轮合计 | **201/201 通过**，0 失败，0 超时，277.05 秒 |
| L3 泛化调用 | 45/45 通过 |
| Connector 桥接调用 | 6/6 通过 |
| GUI/Computer Use | 25 域真实代表路径，68 份界面证据 |
...
| `settings` | 10 | 10/10 通过 | 集中查询、九个稀疏 update、validate-only、GUI/应用状态与恢复 |
...
设置更新只接受公开 allowlist 上的字段。音频设备稀疏更新回归确认未提交字段保持原值；包搜索路径更新在真实环境使用 validate-only，避免替换用户配置。所有实际配置变更和临时歌词规则均在场景结束后恢复。
...
| 文档与工程 | 工程统计、最近项目以及文件类命令的 validate-only/授权路径通过 |
...
| 提取 | capability、短音频 Pitch、MIDI、约 230 秒 DirectML Pitch、取消与成功终态通过 |
| 推理 | capability/status、阶段重置、实际 start Task 与对象结果通过 |
| 异步任务 | list、get、cancel 三项均执行；进度、成功和取消终态可观察 |
...
Computer Use 核对了 Automation 设置页、编辑区域和 MCP 变更后的 GUI 事实，共形成 68 份界面证据。主要结果为：
...
首次故障及其现场仍保留在仓库外私有证据中，但不作为最终通过截图。修复后的同条件成功路径重新执行并纳入最终证据。
...
| 长音频 DirectML Pitch 提取导致进程退出 | RMVPE 将完整连续音频一次送入 ONNX；改为最长 30 秒、前后 1 秒上下文的有界分块，在 10 ms 网格裁剪和拼接 | 取消路径、约 230 秒完整成功、参数点完整回读、Editor 存活及 6 项定向回归通过 |

长音频分块修复位于项目的 synthrt vcpkg overlay，并递增 overlay port version，避免本地旧包继续复用。后续升级 synthrt 或 ONNX Runtime 时需要同步检查补丁适用性；分块边界使用上下文降低边缘影响，但不承诺与一次性超大张量逐位相同。该维护风险不阻断当前候选，真实长输入资格已通过。
...
- 原始构建、定向回归、三轮 CTest、结构化调用、窗口监控、故障现场和 68 份界面证据保存在仓库外私有归档。
...
- [x] 长音频 DirectML 提取故障已修复并以同类真实输入回归。
