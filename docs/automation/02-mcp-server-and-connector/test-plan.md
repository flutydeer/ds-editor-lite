...
当前产品清单为 Editor 176、Connector 6、总计 182，仅作为候选快照记录。测试以权威契约源、Registry、发现面和 Connector 下游面之间的集合关系为门禁，不把工具数量、顺序、顶层场景数或断言数硬编码为正确性证明。CTest 数量由本轮配置与执行产物记录。
...
11. 验证动态值发现、异步任务和文件授权；open/import/import_batch 在任务开始时检查当前读权限，
    带 plan digest 时校验并加载检查器返回的同一字节快照；`formats.inspect` 在解析前拒绝超过
    64 MiB 的输入，MIDI 解析、LibreSVIP 转换和摘要复用同一有界快照；Pitch/MIDI 提取只解码
    一次读取形成的哈希快照，并在提交点复核文档 generation、revision 和目标剪辑身份。
...
15. 验证 `parameters.create_anchor_curve`、显式 `insert_anchors`、跨曲线移动拒绝和 `merge_anchor_curves` 的相邻/重叠规则及逐步 Undo/Redo；验证局部 `parameters.trace` 在采样前拒绝超限或无法表示的锚点展开，且不推进文档版本；验证音符 duplicate 面对延伸至模型 tick 上界的锚点曲线时只采样所选音符交集，并受公共点数上限约束。
...
22. 验证音频导入、重定位和路径确认的 SHA-512 与解码结果来自同一次读取形成的临时快照；后台
    ...
    文档 generation 与应用级作用域的活动任务不被终态历史上限淘汰、只保留最近 128 项终态，且互不影响。
23. 分别模拟文档保存、MIDI 与音频完成覆盖预检后由外部创建同名目标，确认同目录暂存文件的
    排他最终发布不会替换外部文件，也不会暴露部分 DSPX/MIDI；音频多输出逐项原子发布，不提供
    跨输出事务回滚，后续目标失败时保留已完成输出和未被覆盖的外部目标。另验证
    `documents.save` 保留显式拒绝覆盖策略，只有省略策略时才使用当前路径保存的默认覆盖行为；
    音频渲染期间修改同一工程不会丢弃已完成的暂存结果，文档换代和显式取消仍阻止发布。
...
1. 从带图标的选项菜单“自动化”项进入设置页，核对“本地服务器”、无常驻说明的 enabled 卡片、端口、status 和中文翻译，并确认不存在独立的“当前端点”卡片；核对“访问控制”下的连续卡片依次包含“控制层级”“自定义工具集”“文件访问权限”三个条目。
2. Connector 先启动，在 GUI 启用 MCP，观察自动连接与状态更新。
3. 确认一级页不创建或显示领域工具组，且“自定义工具集”条目显示当前 Custom 已启用数/总数；从右侧箭头进入懒加载二级页，再逐组核对 24 个领域卡片默认收起、展开/收起不改变权限、整组关闭/开启、启用计数和单项状态回读，并验证返回导航。切换控制层级/Custom，比较 GUI、Editor list、Connector status 和实际调用；验证“导入控制层级”按所选预设覆盖 Custom，且选择 Custom 时按钮禁用。
