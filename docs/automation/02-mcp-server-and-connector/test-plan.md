...
7. 验证创建深度：轨道只能创建为空轨道，歌声剪辑只能创建为空剪辑，音符叶节点可携带完整初始数据且不要求 voice context；剪辑省略长度时验证跨拍号的四小节默认值和 tick 上界拒绝。
...
22. 验证音频导入、重定位和路径确认的 SHA-512 与解码结果来自同一临时快照；分别验证每个
    文档 generation 与应用级作用域的活动任务不被终态历史上限淘汰、只保留最近 128 项终态，
    且互不影响。
23. 模拟音频导出完成覆盖预检后由外部创建同名目标，确认拒绝覆盖的最终发布不会替换外部文件；
    多文件发布在后续目标冲突时回滚本次已发布的前序文件。
