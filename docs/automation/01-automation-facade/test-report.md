...
当前结论为：**所有已装备且属于一期范围的自动化和 GUI 场景均通过；没有隐藏的自动化失败。**
剩余项目仅为 Computer Use 无法执行的跨窗口拖放、需要人工确认的原生覆盖动作，以及因实际任务
完成过快或缺少安全听音端点而未装备的条件场景。它们不计为通过，也不计为产品失败，统一列在
第 6 节供人工补测。
...
| MIDI/DSPX/LibreSVIP | MIDI 取消/选择/导入、DSPX Open/Import、LibreSVIP 转换、原子回滚和保存通过 |
| 音频资产 | 普通导入、相对路径、Missing Relink、无 hash 同名 Confirm、错误候选拒绝、保存重开通过 |
| 推理与提取 | 零缓存 4 轨/8 Clip 推理、受控延迟 Undo、RMVPE Pitch、GAME MIDI、单步 Undo 通过 |
| MIDI 导出 | Cancel/No 不覆盖、新文件导出、独立解析和 round-trip 通过；Confirm/Yes 留人工补测 |
| 应用生命周期 | 两次真实 Restart、单进程/单窗口、窗口模式恢复、Exit Cancel、干净退出和最终清理通过 |
...
以下场景没有被自动化结果冒充为通过。请只使用工作副本和可丢弃输出文件。

### M01：工程与音频混合跨窗口拖放

- [ ] 在文件管理器中同时选择一个可导入工程和一个有效短音频，拖到编辑器工作区。
...
- [ ] 把一个明确不支持的普通文本文件拖入编辑器。
- 预期：显示不支持/无法导入提示；Model、History、revision、Recent 和文件内容均不变。

### M03：MIDI 已存在目标确认覆盖

- [ ] 准备一个可丢弃、内容与当前工程不同的既有 MIDI，先记录大小和 SHA-256。
- [ ] 执行 File → Export → MIDI file，选择该文件，并在原生确认框选择 Yes/Confirm。
- 预期：文件被一次性替换为非空、可解析的 MIDI，大小或 SHA-256 改变；没有临时文件或半写入。
- [ ] 新建工程，导入覆盖后的 MIDI 并勾选全部轨、Tempo 和拍号。
- 预期：轨数、音符位置、可表示歌词、Tempo 和拍号与源工程一致。

### M04：零缓存 8 Clip 逐项听音

- [ ] 在安全音频端点下清空测试缓存，打开 4 条歌声轨、每轨 2 个 Clip 的推理工程。
- [ ] 等待 8 个 Clip 均为 Success；逐轨 Solo 或静音其他轨，把播放头放入每个 Clip 后逐一播放。
- 预期：每个 Clip 均有与自身内容对应的非静音声音，电平和播放头正常；无静音、串轨、旧结果、
  持续闪烁或红色失败段。

### M05：长音频解析/解码取消（条件场景）

- [ ] 仅在能稳定显示可取消任务框时，导入足够慢的有效长音频并在 Running 阶段 Cancel。
- 预期：任务终态为 Canceled；不留下空轨、空 Clip、旧音频、波形 cache、Missing/Normal 错乱或
  后续晚到写回。
- 若本机任务仍在可操作前完成，记录为“环境未具备”，不要用完成后的 Undo 代替 Cancel。

### M06：Pitch/MIDI 提取取消（条件场景）

- [ ] 分别对足够长的音频启动 Extract Pitch 和 Extract MIDI，在 Running 阶段各 Cancel 一次。
- 预期：Pitch 不留下半条曲线；MIDI 不留下半条歌声轨或部分音符；任务稳定为 Canceled，随后可
  重新执行并成功完成。
- 若模型在可操作前完成，记录为“环境未具备”。

### M07：运行任务期间退出（条件场景）

- [ ] 在可稳定保持 Running 的专用任务中请求 File → Exit。
