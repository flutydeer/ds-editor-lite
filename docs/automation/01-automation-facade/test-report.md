...
最终测试树基于提交 `66b42c60`；本报告整理只改变文档，不改变已测试的生产代码或测试代码。
...
| 注册 CTest | 53 |
| 完整 CTest 第 1 轮 | 53/53 通过，0 失败，0 超时，8.35 秒 |
| 完整 CTest 第 2 轮 | 53/53 通过，0 失败，0 超时，8.37 秒 |
| 完整 CTest 第 3 轮 | 53/53 通过，0 失败，0 超时，8.23 秒 |
...
- Clip 幂等指纹覆盖参数曲线集合边界、音频路径状态、实时锚点和完整派生缓存结构；任一生效字段
  或结构变化均稳定冲突。
- Voice Context 幂等指纹覆盖 Singer 名称/本地化名、语言与 G2P、解析状态、capability、Speaker
  音域/可混合性以及 Speaker Mix source 内完整 Speaker；Track、Clip、Document、Import 与
  Speaker Mix 命令的元数据差异均稳定冲突。
