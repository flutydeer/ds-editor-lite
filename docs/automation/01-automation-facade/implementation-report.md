...
- 音频解析、解码、哈希和状态写回使用 source generation 与各操作实际依赖的指纹；导入、重定位
  和路径确认的哈希与解码读取同一份临时快照，音频导入还会在提交前于后台重算源文件摘要并与
  快照摘要核对；可重建缓存不增 revision/History，持久化结果仍遵守统一提交规则。
- Pitch/MIDI 提取在异步后端启动前和进入不可取消的 Committing 前分别复核冻结的源路径授权；
  运行期间撤销读权限会使任务失败，不写回参数、音符、剪辑或轨道。
...
- 补齐片段材料范围、音素偏移数量/单调性、参数 Draw step、量化网格等领域验证；量化通过共享
  时间轴宽整数计算候选起点与长度，在缩窄到模型 tick 前拒绝越界结果，并消除修改遍历容器导致的
  越界/挂起。
...
- 文档保存、MIDI 与音频导出把序列化或渲染与最终发布分开；拒绝覆盖时先写同目录暂存文件，再以
  ...
  MIDI/音频最终发布前仍复核 generation 与授权，批量音频发布失败时回滚本次已经发布的文件。
