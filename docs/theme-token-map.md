...
| Token | 职责 |
|---|---|
| `timeline.ruler.barTick` | 小节刻度线 |
| `timeline.ruler.beatScale` | 拍号刻度文本 |
| `timeline.ruler.beatTick` | 拍刻度线 |
| `timeline.ruler.subdivisionFrom` | 细分网格渐变起点 |
| `timeline.ruler.subdivisionTo` | 细分网格渐变终点 |

### Phoneme、Speaker Mix 与 Mixer

| Token | 职责 |
|---|---|
| `phoneme.hint` | 音素视图提示文本 |
| `phoneme.text` | 音素视图正文 |
| `phoneme.positionLine` | 音素位置线 |
| `phoneme.noteBoundary` | 音素视图音符边界 |
| `phoneme.waveform` | 音素视图波形 |
| `speakerMix.position.text` | 说话人位置标签文本 |
| `speakerMix.position.fill` | 说话人位置标签背景 |
| `speakerMix.position.border` | 说话人位置标签边框 |
| `speakerMix.track` | 说话人混合条轨道 |
| `speakerMix.segmentText` | 说话人混合色块上的文本 |
| `speakerMix.divider` | 说话人混合条分隔线 |
...
| 前缀 | 后缀 |
|---|---|
| `button.mute` | `checked.foreground`、`checked.fill`、`checkedHover.fill`、`checkedPressed.fill` |
| `button.solo` | `checked.foreground`、`checked.fill`、`checkedHover.fill`、`checkedPressed.fill` |

这套变体供轨道、片段和混音台共享，不绑定到 `mixer` 命名空间。禁用状态先回退到通用
`checkedDisabled`，只有发现 Mute/Solo 需要特殊禁用表现时才增加组件 token。

Mute/Solo 当前采用无描边组件契约。`control.border.*` 表示可使用的颜色职责，不要求所有控件
都声明 border；需要无描边的控件应直接使用 `border: none`，不使用透明描边模拟。
