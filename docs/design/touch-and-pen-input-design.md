...
Windows 的长按转右键还会额外送一个 `QContextMenuEvent`（`reason() == Mouse`），与手势层自己投递的菜单撞车。真机日志确认了它的时序：手指按住约 900 毫秒后抬起，Windows 在**抬起时**才补上右键按下、抬起和菜单事件，而手势层的长按早在 450 毫秒时就已经触发，于是会开出两个菜单。

控件内在触摸活跃期间及其后一段宽限时间里吞掉 `reason() == Mouse` 的菜单事件，手势层自己投递的用 `Other`，键盘菜单键用 `Keyboard`，两者都放行。
