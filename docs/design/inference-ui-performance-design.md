...
音频后端的 `positionChanged` 到达间隔并不均匀，视图各自使用单次定时器节流会把相邻事件合并，无法形成稳定的视觉刷新节奏。`PlaybackController` 因此保留 `positionChanged` 作为音频、推理等逻辑的权威位置，同时提供由 8ms 精确定时器驱动的 `visualPositionChanged`。播放期间以最近一次权威位置为锚点，按时间轴的速度曲线插值得到视觉位置；暂停、停止或手动定位时则立即同步。

Legacy 视图直接消费统一的视觉位置信号，不再维护各自的播放位置节流器，并以视口子控件单独移动播放头，避免触发整幅场景重绘。RHI 视图进一步将当前播放头从完整场景快照中拆出为独立动态叠加层，每次视觉更新只上传一条竖线的顶点；音符、曲线、波形、文本和末次播放位置仍由静态帧管理。静态帧在更新前回收，绘制命令和字形顶点批次就地复用已分配的存储，避免隐式共享容器在下一帧写入时持续分离和扩容。
