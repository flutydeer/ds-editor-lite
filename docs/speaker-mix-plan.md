...
- `Dynamic Mix` 的关键帧编辑器已有视图和交互 demo，但尚未绑定 `SingingClip::SpeakerMixData`。
...
- UI 分阶段落地：
    ...
    - 下一步：把 `SpeakerMixEditorView` 从硬编码数据改为绑定 `SingingClip::SpeakerMixData`，关键帧编辑通过 action 提交。
...
- Phase 1：正式化比例控件 — **基本完成**
    ...
    - 🔲 等 Dynamic Mix 接入后，再完成固定模式与关键帧模式的权限切换。

- Phase 2：Clip 模型与撤销重做 — **Fixed Mix 已完成，Dynamic Mix 待接入**
    - ✅ 在 `SingingClip` 增加 `SpeakerMixData`。
    - ✅ 新增 `ReplaceSpeakerMixAction` / `SpeakerMixActions`。
    - ✅ Fixed Mix 对话框 OK 后通过 action 提交。
    - 🔲 调整 normalize 语义：FixedMix 保留 `dynamicKeyframes`，DynamicMix 保留 `fixedWeights`，让 Dynamic Mix 开关可恢复 inactive 数据。
...
- Phase 3：DSPX / workspace 读写 — **workspace 已完成**
    ...
    - 🔲 Dynamic Mix 写入/读取 `dynamicKeyframes` 后，需要用真实编辑器验证。
    - 🔲 opendspx 官方 mix 结构暂缓。

- Phase 4：Dynamic Mix 编辑闭环 — **下一步主线**
