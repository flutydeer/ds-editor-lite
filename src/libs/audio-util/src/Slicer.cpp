...
namespace AudioUtil
{
    ...
    template <class T>
    inline std::vector<double> get_rms(const std::vector<T> &arr, uint64_t frame_length = 2048,
                                       uint64_t hop_length = 512);
    ...
    Slicer::Slicer(SndfileHandle *decoder, double threshold, uint64_t minLength, uint64_t minInterval, uint64_t hopSize,
        ...
        m_winSize =
            std::min(divIntRound<uint64_t>(minInterval * (uint64_t)sr, (uint64_t)1000), (uint64_t)4 * m_hopSize);
