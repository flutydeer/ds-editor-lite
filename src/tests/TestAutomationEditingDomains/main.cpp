...
class TestAutomationEditingDomains final : public QObject {
    Q_OBJECT
...
private slots:
    ...
    void trackEditing() {
        ...
        TrackId third;
        {
            ...
            const auto insert = runtime.project().insertTrack(
                ...
                trackDraft(QStringLiteral("First"), QStringLiteral("track-first")));
            QVERIFY2(
                (insert && insert.get().current.revision == initial.revision + 1 &&
...

        {
            ...
            QVERIFY2(
                (noOp && !noOp.get().changed && runtime.documentVersion() == move.get().current),
            ...
            QVERIFY2((undo && undo.get().changed && restored &&
            ...
            QVERIFY2((endPreview && endPreview.get().changed && endPreview.get().validatedOnly &&
...

        {
            ...
            QVERIFY2(
                (isError(empty, AutomationErrorCode::InvalidArgument, QStringLiteral("language"))),
            ...
            const auto state = runtime.history().getState(runtime.documentVersion().documentId);
            QVERIFY2(
                (noOp && !noOp.get().changed && state && state.get().canUndo),
    ...
    void singingClipEditing() {
        ...
        ClipId clip;
        {
            ...
            QVERIFY2(
                (isError(noteOverflow, AutomationErrorCode::InvalidArgument,
...

        {
            ...
            auto farDraft =
                singingClipDraft(QStringLiteral("Far Clip"), QStringLiteral("clip-far"));
            ...
            QVERIFY2((isError(preview, AutomationErrorCode::InvalidArgument,
                      ...
                      runtime.documentVersion() == base),
                     qPrintable(QStringLiteral(
                         "translated duplicate ranges must be checked before preview or "
                         "commit")));
        };
...

        {
            ...
            QVERIFY2((isError(rejectedLocalOverflow, AutomationErrorCode::InvalidArgument,
                              QStringLiteral("properties"))),
                     qPrintable(QStringLiteral(
                         "clip edit must reject an unrepresentable clip-local end")));
            const auto base = runtime.documentVersion();
            const auto preview =
...

        {
            ...
            QVERIFY2(
                (isError(blank, AutomationErrorCode::InvalidArgument, QStringLiteral("language"))),
...

        {
            ...
            QVERIFY2((isError(duplicate, AutomationErrorCode::InvalidArgument,
    ...
    void legacyAudioClipEditing() {
        ...
        ClipId legacyAudioClip;
        {
            ...
            QVERIFY2((insert && snapshot &&
                     ...
                     qPrintable(QStringLiteral(
                         "legacy audio geometry must be inserted without normalization")));
            if (!snapshot)
                return;
...

        {
            ...
            QVERIFY2((undoMove && movedBack && movedBack->trackId == second &&
                      sameClipTiming(movedBack->data.properties, before->data.properties)),
                     qPrintable(QStringLiteral(
                         "undoing a cross-track move must preserve legacy geometry")));
        };

        {
            ...
            QVERIFY2((changed && undo && restored && restored->trackId == second &&
                      sameClipTiming(restored->data.properties, before->data.properties)),
                     qPrintable(QStringLiteral(
                         "undoing an audio timing edit must restore raw legacy ticks")));
        };

        {
            ...
            QVERIFY2((changed && undo && restored && restored->trackId == second &&
                      sameClipTiming(restored->data.properties, before->data.properties)),
                     qPrintable(QStringLiteral(
                         "undoing an audio timing move must restore raw legacy ticks")));
        };
    }
    ...
    void trackRemovalRestoresChildren() {
        ...
        QVERIFY(clip.isValid());
        {
            ...
            QVERIFY2((explicitZeroInsert && explicitZeroRemove && explicitZeroUndo &&
                      explicitZeroRestored && explicitZeroRestored->data.colorIndex == 0),
                     qPrintable(QStringLiteral(
    ...
    void listNotes() {
        ...
        QVERIFY2((notes && notes.get().size() == 2 &&
    ...
    void insertNotes() {
        ...
        QVERIFY2(
            (insert && insert.get().createdObjects.size() == 1 &&
        ...
        const auto overlapPreview = runtime.notes().insertNotes(commandContext(runtime, true),
    ...
    void moveNotes() {
        ...
        QVERIFY2(
            (isError(duplicate, AutomationErrorCode::InvalidArgument, QStringLiteral("note_ids"))),
        ...
        QVERIFY2((isError(invalidKey, AutomationErrorCode::InvalidArgument,
                          QStringLiteral("delta_key"))),
                 qPrintable(QStringLiteral("out-of-range key must be rejected")));
        const auto overflowingTick =
            runtime.notes().moveNotes(commandContext(runtime), fixture.clipId,
                                      {fixture.firstNoteId}, std::numeric_limits<int>::max(), 0);
        const auto overflowingKey =
            runtime.notes().moveNotes(commandContext(runtime), fixture.clipId,
                                      {fixture.firstNoteId}, 0, std::numeric_limits<int>::max());
        ...
        QVERIFY2(
            (undo && restored && restored->data.localStart == 73 && restored->data.keyIndex == 60),
    ...
    void resizeNotesLeft() {
        ...
        const auto duplicate =
            runtime.notes().resizeNotesLeft(commandContext(runtime), fixture.clipId,
                                            {fixture.firstNoteId, fixture.firstNoteId}, 10, 1);
        QVERIFY2(
            (isError(duplicate, AutomationErrorCode::InvalidArgument, QStringLiteral("resize"))),
    ...
    void resizeNotesRight() {
        ...
        QVERIFY2((isError(overflowing, AutomationErrorCode::InvalidArgument,
                          QStringLiteral("delta_tick")) &&
                  runtime.documentVersion() == base),
    ...
    void splitNote() {
        ...
        const auto invalid =
            runtime.notes().splitNote(commandContext(runtime), fixture.clipId, fixture.secondNoteId,
                                      child, before->data.length);
        ...
        const auto base = runtime.documentVersion();
        const auto preview = runtime.notes().splitNote(
            commandContext(runtime, true), fixture.clipId, fixture.secondNoteId, child, 180);
        QVERIFY2((preview && preview.get().validatedOnly && preview.get().changed &&
                  preview.get().createdObjects.isEmpty() && runtime.documentVersion() == base),
    ...
    void resetPhonemeOffsetsCascades() {
        ...
        QVERIFY2(
            (undo && restoredFirst && restoredSecond &&
    ...
    void setWordProperties() {
        ...
        QVERIFY2(
            (isError(duplicate, AutomationErrorCode::InvalidArgument, QStringLiteral("edits"))),
        ...
        const auto base = runtime.documentVersion();
        const auto preview = runtime.notes().setWordProperties(commandContext(runtime, true),
                                                               fixture.clipId, {edit});
        QVERIFY2((preview && preview.get().changed && preview.get().validatedOnly &&
                  runtime.documentVersion() == base),
    ...
    void removeNotes() {
        ...
        fixture.testRuntime.history()->reset();
        const auto duplicate = runtime.notes().removeNotes(
            commandContext(runtime), fixture.clipId, {fixture.firstNoteId, fixture.firstNoteId});
        QVERIFY2(
            (isError(duplicate, AutomationErrorCode::InvalidArgument, QStringLiteral("note_ids"))),
        ...
        const auto base = runtime.documentVersion();
        const auto preview = runtime.notes().removeNotes(commandContext(runtime, true),
                                                         fixture.clipId, {fixture.secondNoteId});
        QVERIFY2((preview && preview.get().changed && preview.get().validatedOnly &&
        ...
        QVERIFY2(
            (removed && !noteSnapshot(runtime, fixture.clipId, fixture.secondNoteId).has_value()),
    ...
    void curveTransforms() {
                                                      ...
                                                      : Automation::OperationIds::parameters::scale;
        {
            ...
            testRuntime.history()->reset();
            const Automation::ParameterTransformDto request{1, 99, kind == Kind::Scale ? 0.5 : 0.0,
                                                            1, 99};
            ...
            QVERIFY2(
                (noOp && !noOp.get().changed && noOp.get().resolvedValues == bounds),
    ...
    void parameterEditing() {
...

        {
            ...
            QVERIFY2((isError(overflow, AutomationErrorCode::InvalidArgument,
...

        {
            ...
            const auto traced = runtime.parameters().traceParameter(commandContext(runtime), clipId,
                                                                    ParamInfo::Pitch, 0, 5);
            QVERIFY2(
                (traced && !traced.get().changed && runtime.documentVersion() == base),
            ...
            QVERIFY2((applied && applied.get().changed &&
                      ...
                      after.size() == 2 &&
                      after.first().values ==
                          QList<int>{6100, 6110, 6000, 6000, 6200, 6210, 6000, 6000} &&
    ...
    void speakerMixEditing() {
...

        {
            ...
            QVERIFY2((changed && changed.get().current.revision == base.revision + 1 && snapshot &&
                      snapshot->data.singerInfo == singerA &&
...

        {
            ...
            QVERIFY2(
                (changed && changed.get().current.revision == base.revision + 1 && snapshot &&
                 !snapshot->data.usesTrackVoiceContext && snapshot->data.ownSingerInfo == singerA &&
...

        {
            ...
            QVERIFY2(
                (changed && changed.get().current.revision == base.revision + 1 && before &&
                 after && after->data.ownSingerInfo == before->data.ownSingerInfo &&
    ...
    void timelineAndHistoryDomain() {
...

        {
            ...
            QVERIFY2(
                (timeline && timeline.get().document == runtime.documentVersion() &&
...

        {
            ...
            QVERIFY2((isError(anchor, AutomationErrorCode::InvalidArgument,
...

        {
            ...
            QVERIFY2(
                (empty && !empty.get().canUndo && !empty.get().canRedo && empty.get().onSavePoint),
...

        {
            ...
            const auto undo = runtime.history().undo(commandContext(runtime));
            QVERIFY2(
                (undo && undo.get().changed && undo.get().current.revision == before.revision + 1),
...

        {
            ...
            const auto redo = runtime.history().redo(commandContext(runtime));
            QVERIFY2(
                (redo && redo.get().changed && redo.get().current.revision == before.revision + 1),
...
int main(int argc, char *argv[]) {
    ...
    QCoreApplication application(argc, argv);
    TestAutomationEditingDomains tests;
