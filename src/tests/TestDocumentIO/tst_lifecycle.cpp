...
void DocumentIOTests::initialUntitledSession() {
    ...
    QVERIFY2((version.revision == 0), "the initial untitled session must start at revision zero");
...
void DocumentIOTests::newOpenAndImport() {
    ...
    QVERIFY2((committedNew && committedNew.get().changed &&
              committedNew.get().previous == initial && committedNew.get().current == afterNew &&
    ...
    auto emptyImport = makeDocumentDraft(QStringLiteral("unused"), QStringLiteral("empty-import"));
    emptyImport.tracks.clear();
    const auto beforeEmptyImport = runtime.documentVersion();
    const auto noOpImport = runtime.documents().commitImportedDocument(commandContext(runtime),
                                                                       emptyImport, false, false);
    QVERIFY2(
        (noOpImport && !noOpImport.get().changed && runtime.documentVersion() == beforeEmptyImport),
...
void DocumentIOTests::commitObservers() {
    ...
    QVERIFY2(
        (saved && resaved && fixture.host.commits.size() == 4 &&
         fixture.host.commits.at(2).operationId == Automation::OperationIds::documents::save_as &&
...
void DocumentIOTests::workflowBusyLeaseAcrossReplacement() {
    ...
    const auto replacement = runtime.documents().commitOpenedDocument(
        continuation,
        makeDocumentDraft(QStringLiteral("Busy Replacement"), QStringLiteral("busy-replacement")),
        QStringLiteral("fixtures/busy-replacement.dspx"), QStringLiteral("busy-replacement.dspx"),
        true);
    ...
    QVERIFY2((admitted && acquired && replacement && current.documentId != original.documentId &&
...
void DocumentIOTests::failureAndCancellationRollback() {
    ...
    const auto task = runtime.automationTasks().createTask(
        Automation::OperationIds::extract::pitch::start, runtime.documentVersion(), std::nullopt,
        [&taskCancelCount] { ++taskCancelCount; });
    ...
    QVERIFY2(
        (!invalidReplacement &&
         invalidReplacement.getError().code == Automation::AutomationErrorCode::InvalidArgument &&
    ...
    const auto interveningEdit = runtime.timeline().setTempo(commandContext(runtime), 960, 132.0);
    ...
    QVERIFY2((interveningEdit && !staleCommit &&
...
void DocumentIOTests::saveAndSaveAs() {
    ...
    const auto failedSave = runtime.documents().saveDocument(commandContext(runtime), failedPath);
    ...
    const auto retriedSave = runtime.documents().saveDocument(commandContext(runtime), failedPath);
    ...
    QVERIFY2((retriedSave && runtime.documentVersion() == failedVersion &&
              fixture.host.saveCalls == 4 && afterRetry && afterRetry.get().path == failedPath &&
...
void DocumentIOTests::generationCleanup() {
    ...
    const auto committedNew = runtime.documents().commitNewDocument(
        commandContext(runtime),
        makeDocumentDraft(QStringLiteral("Cleanup Baseline"), QStringLiteral("cleanup-baseline")));
    ...
    const auto runningTask = runtime.automationTasks().createTask(
        Automation::OperationIds::extract::pitch::start, runtime.documentVersion(), std::nullopt,
        [&runningTaskCancelCount] { ++runningTaskCancelCount; });
    ...
    QVERIFY2((committedNew && firstImport && firstInsert && secondImport && undo && historyBefore &&
    ...
    QVERIFY2((replacement && newVersion.documentId != oldVersion.documentId &&
              ...
              !historyAfter.get().canRedo && historyAfter.get().onSavePoint && idempotencyAfter &&
              idempotencyAfter.get() == 0 && runtime.automationTasks().size() == 0 &&
              runningTaskCancelCount == 1 &&
...
void DocumentIOTests::oldIdCollisionAndErrorPriority() {
    ...
    QVERIFY2((index.contains(oldDocument, 37) && index.contains(currentSession.documentId(), 37) &&
              !collision &&
    ...
    const auto committedNew = runtime.documents().commitNewDocument(
        commandContext(runtime),
        makeDocumentDraft(QStringLiteral("Priority Old Track"), QStringLiteral("priority-old")));
    const auto oldVersion = runtime.documentVersion();
    const auto replacement = runtime.documents().commitOpenedDocument(
        commandContext(runtime),
        makeDocumentDraft(QStringLiteral("Priority Current Track"),
                          QStringLiteral("priority-current")),
        QStringLiteral("fixtures/priority-current.dspx"), QStringLiteral("priority-current.dspx"),
        true);
    ...
    QVERIFY2(
        (committedNew && replacement && currentProject && !currentProject.get().tracks.isEmpty()),
    ...
    QVERIFY2((documentError.getError().operationId == Automation::OperationIds::tracks::set_color &&
