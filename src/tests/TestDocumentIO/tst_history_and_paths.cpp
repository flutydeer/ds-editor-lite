...
void DocumentIOTests::init() {
    historyManager->reset(HistoryManager::ResetState::Saved);
}

void DocumentIOTests::cleanup() {
    historyManager->reset(HistoryManager::ResetState::Saved);
}

void DocumentIOTests::savePointBaseline() {
...
void DocumentIOTests::importSequenceUndoRedoAndOwnership() {
...
void DocumentIOTests::historyNamesFollowTranslator() {
    const auto counts = std::make_shared<ActionCounts>();
    auto sequence = new TestActionSequence;
    sequence->setTranslatableName("TestActionSequence",
                                  QT_TRANSLATE_NOOP("TestActionSequence", "Translatable action"));
    sequence->addAction(new CountingAction(counts));
    historyManager->record(sequence);
...
void DocumentIOTests::discardedSavedBranchCannotBecomeCleanThroughAddressReuse() {
...
void DocumentIOTests::guardedTransition() {
    ...
    auto trueTransition =
        new ConditionalTransition(&emitter, SIGNAL(proceed()), [condition] { return condition; });
    trueTransition->setTargetState(whenTrue);
    initial->addTransition(trueTransition);
    auto falseTransition =
        new ConditionalTransition(&emitter, SIGNAL(proceed()), [condition] { return !condition; });
...
void DocumentIOTests::suggestedSavePath_data() {
    QTest::addColumn<QString>("name");
    QTest::addColumn<QString>("expected");
    QTest::newRow("plain") << QStringLiteral("test") << QStringLiteral("test.dspx");
    QTest::newRow("multiple-dots") << QStringLiteral("song.v1") << QStringLiteral("song.v1.dspx");
    QTest::newRow("spaces") << QStringLiteral("New Project") << QStringLiteral("New Project.dspx");
    QTest::newRow("existing-suffix")
        << QStringLiteral("draft.dspx") << QStringLiteral("draft.dspx");
    QTest::newRow("uppercase-suffix")
        << QStringLiteral("draft.DSPX") << QStringLiteral("draft.DSPX");
    QTest::newRow("unicode") << QStringLiteral("未命名作品") << QStringLiteral("未命名作品.dspx");
}

void DocumentIOTests::suggestedSavePath() {
    QFETCH(QString, name);
...
void DocumentIOTests::existingSavePathIsPreserved() {
    const auto existing = QDir(QDir::tempPath()).filePath(QStringLiteral("existing.project.dspx"));
    QCOMPARE(DocumentWorkflowPathUtils::suggestedSavePath(existing, QStringLiteral("ignored"),
                                                          QStringLiteral("ignored")),
             existing);
}

void DocumentIOTests::projectPathCaseSensitivity() {
...
void DocumentIOTests::dirtyRevisionMustBeApprovedAgain() {
...
#include "tst_history_and_paths.moc"
