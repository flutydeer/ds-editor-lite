...
void DocumentIOTests::batchPreparationFailures() {
    {
        ...
        QVERIFY2((prepared.size() == 3),
                 qPrintable(QStringLiteral("batch preparation must preserve input cardinality")));
        if (prepared.size() != 3)
            return;

        QVERIFY2((prepared.at(0).kind == PreparedImportItem::Kind::Failed &&
        ...
        QVERIFY2(
            (invalidSummary.contains(QFileInfo(invalidPath).fileName()) &&
             invalidSummary.contains(prepared.at(0).errorMessage)),
            qPrintable(QStringLiteral("invalid MIDI summary must identify the item and reason")));
        QVERIFY2(
            (emptySummary.contains(QFileInfo(emptyPath).fileName()) &&
             emptySummary.contains(prepared.at(1).errorMessage)),
            qPrintable(QStringLiteral("note-free MIDI summary must identify the item and reason")));
        QVERIFY2(
            (MidiFilePreparer::failureMessage(prepared.at(2)).isEmpty()),
            qPrintable(QStringLiteral("successful MIDI items must not enter the failure summary")));
    }
}

void DocumentIOTests::selectionAndGeometry() {
    {
        ...
        QVERIFY2(
            (generated.hasTimeline && generated.tempos.size() == 1 &&
             generated.timeSignatures.size() == 1),
            qPrintable(QStringLiteral("selected import options must preserve the MIDI timeline")));

        const auto draft = makeImportDraft(generated);
        QVERIFY2((draft.tracks.size() == 1 && draft.tracks.first().name == QStringLiteral("英语") &&
        ...
        QVERIFY2(
            ...
            qPrintable(QStringLiteral("visible MIDI clip geometry must fit its material length")));
        const auto validation = Automation::validate(draft);
        QVERIFY2(
            (static_cast<bool>(validation)),
            qPrintable(QStringLiteral("generated MIDI document must satisfy Facade validation")));
    }
...
void DocumentIOTests::facadeCommitUndoRedo() {
...

    {
        ...
        const auto result =
            runtime.documents().commitImportedDocument(context(runtime, true), draft, true, true);
...

    {
        ...
        QVERIFY2(
            ...
            qPrintable(QStringLiteral("confirmed timeline options must be committed atomically")));
...

    {
        ...
        QVERIFY2(
            ...
            qPrintable(QStringLiteral("one undo must remove the track and restore the timeline")));
...
void DocumentIOTests::preparedBatchCommitBoundaries() {
    AppModel destination;
    destination.newProject();
    auto *history = HistoryManager::instance();
    history->reset(HistoryManager::ResetState::Saved);
...

    {
        ...
        const auto committed = commitContext
...

    {
        ...
        const auto rejected = runtime.project().commitBatchImport(context(runtime), missingTarget);
        QVERIFY2(
            (!rejected && rejected.getError().code == Automation::AutomationErrorCode::NotFound &&
             runtime.documentVersion() == before),
            qPrintable(QStringLiteral("a deleted destination track must reject the whole batch")));
    }
}
