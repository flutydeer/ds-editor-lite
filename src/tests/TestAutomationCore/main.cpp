...
int main(int argc, char *argv[]) {
    ...
    const auto createdObject =
        [&](const QString &clientRef) -> std::optional<Automation::ObjectRef> {
        if (!midiTask || !midiTask.get().mutation)
            return std::nullopt;
        const auto &created = midiTask.get().mutation->createdObjects;
        const auto found = std::find_if(created.cbegin(), created.cend(), [&](const auto &item) {
