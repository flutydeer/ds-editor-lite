...
class TestAutomationAsyncFileDomains final : public QObject {
    Q_OBJECT
...
private slots:
    ...
    void inferenceMatrix() {
        ...
        QVERIFY2((harness.isReady()),
                 qPrintable(QStringLiteral("inference harness must initialize")));
...

        {
            ...
            QVERIFY2((preview && preview.get().mutation.validatedOnly &&
                     ...
                     qPrintable(QStringLiteral(
                         "validate-only must predict without applying or mutating")));
            ...
            QVERIFY2((noOp && !noOp.get().mutation.changed &&
    ...
    void inferenceValidationBoundaries() {
...

        {
            ...
            QVERIFY2((isError(rejectedDocument, Automation::AutomationErrorCode::DocumentChanged,
            ...
            QVERIFY2((isError(missingPiece, Automation::AutomationErrorCode::NotFound,
                              testCase.operationId) &&
                      isError(missingNote, Automation::AutomationErrorCode::NotFound,
                              testCase.operationId)),
                     qPrintable(QStringLiteral(
                         "piece and note failures must be typed and operation-scoped")));
...

        {
            ...
            QVERIFY2((workflowContext &&
                      workflowContext.get().expected == runtime.documentVersion() &&
            ...
            QVERIFY2((replacement &&
                     ...
                     qPrintable(QStringLiteral(
                         "late writeback must not enter services after replacement")));
        };
    ...
    void audioClipDomain() {
        RuntimeHarness harness;
        QVERIFY2((harness.isReady()),
                 qPrintable(QStringLiteral("audio clip harness must initialize")));
        auto &runtime = harness.runtime();
        auto *audio =
...

        {
            ...
            const auto commit =
                runtime.project().setAudioClipPathStatus(harness.context(), harness.audioClipId(),
            ...
            QVERIFY2((preview && preview.get().validatedOnly && commit && commit.get().changed &&
...

        {
            ...
            QVERIFY2((preview && preview.get().validatedOnly && commit &&
                      ...
                      isError(wrongType, Automation::AutomationErrorCode::WrongObjectType,
                              Automation::OperationIds::audio_clips::relocate)),
                     qPrintable(QStringLiteral(
                         "relocation must be one history revision with typed errors")));
        };

        {
            ...
            QVERIFY2(
                (preview && preview.get().validatedOnly && preview.get().changed &&
...

        {
            ...
            QVERIFY2((derived && preview && preview.get().validatedOnly && commit &&
                      commit.get().current.revision == base.revision + 1 && noOp &&
                      !noOp.get().changed && audio->pathStatus() == AudioClip::PathStatus::Normal &&
                      isError(wrongType, Automation::AutomationErrorCode::WrongObjectType,
                              Automation::OperationIds::audio_clips::confirm_path)),
                     qPrintable(QStringLiteral(
                         "path confirmation must commit once and type-check clips")));
        };
    }
    ...
    void documentAndImportDomains() {
        {
            ...
            auto &runtime = harness.runtime();
            {
                ...
                const auto commit = runtime.documents().commitImportedDocument(harness.context(),
                                                                               draft, false, false);
...

        {
            RuntimeHarness harness;
            QVERIFY2((harness.isReady()),
                     qPrintable(QStringLiteral("save harness must initialize")));
            auto &runtime = harness.runtime();
            {
    ...
    void formatsAndMidiExport() {
...

        {
            ...
            const auto direct = unavailable.runtime()
                                    .dispatcher()
            ...
            QVERIFY2((!missing), qPrintable(QStringLiteral(
                                     "missing format service must return an error result")));
            QVERIFY2(
                (!missing &&
            ...
            QVERIFY2((!missing &&
                      missing.getError().operationId == Automation::OperationIds::formats::list),
                     qPrintable(QStringLiteral("missing format operation ID is '%1'")
            ...
            QVERIFY2((!direct &&
                     ...
                     qPrintable(QStringLiteral("direct query operation ID is '%1'")
                                    .arg(direct ? QStringLiteral("<success>")
                                                : direct.getError().operationId)));
        };

        {
            ...
            QVERIFY2(
                (directPreview && directPreview.get().validatedOnly &&
            ...
            const auto preparedWrite =
                prepared ? runtime.files().writePreparedMidiExport(prepared.get())
                         : Automation::AutomationResult<Automation::FileWriteResultDto>(
                               prepared.getError());
            QVERIFY2(
                (prepared && prepared.get().modelSnapshot.tracks.size() == 1 &&
            ...
            const auto relative = runtime.files().exportMidi(harness.context(),
                                                             QStringLiteral("relative.mid"), false);
            ...
            QVERIFY2(
                (isError(backendFailure, Automation::AutomationErrorCode::IoError,
                         Automation::OperationIds::exports::midi::start) &&
    ...
    void audioExportAndTaskList() {
...

        {
            ...
            QVERIFY2((!missing), qPrintable(QStringLiteral(
                                     "missing audio service must return an error result")));
            QVERIFY2((!missing &&
                      missing.getError().code == Automation::AutomationErrorCode::ModuleNotReady),
        ...
        Automation::TaskId acceptedTask;
        {
            ...
            QVERIFY2((validation && validation.get().validatedOnly &&
                     ...
                     qPrintable(QStringLiteral(
                         "validation must not allocate; start must queue one task")));
            const auto base = runtime.documentVersion();
            ...
            QVERIFY2((harness.audioExportState()->cleanupCount == 1),
                     qPrintable(
            ...
            QVERIFY2((failedAccepted && failedTask &&
                     ...
                     qPrintable(QStringLiteral(
                         "audio backend failure must remain queryable and force cleanup "
                         "exactly once")));
        };

        {
            ...
            const auto deniedTask =
                denied ? runtime.tasks().getTask(runtime.documentVersion().documentId,
                       ...
                       : Automation::AutomationResult<Automation::AutomationTaskSnapshot>(
                             Automation::AutomationError{});
            QVERIFY2((denied && deniedRan && deniedTask &&
                     ...
                     qPrintable(QStringLiteral(
                         "failed final authorization must discard staged audio without "
                         "publishing")));
...

        {
            ...
            const auto terminal =
                accepted ? runtime.tasks().getTask(base.documentId, accepted.get().taskId)
                         : Automation::AutomationResult<Automation::AutomationTaskSnapshot>(
                               Automation::AutomationError{});
            QVERIFY2((accepted && ran && editedDuringRender && terminal &&
                      terminal.get().state == Automation::AutomationTaskState::Succeeded &&
                      state->executeCount == executeBefore + 1 && state->deferPublish &&
                      state->publishCount == publishBefore + 1 &&
                     ...
                     qPrintable(QStringLiteral(
                         "document edits during audio rendering must not discard the "
                         "completed export")));
        };
    ...
    void extractionDomains() {
        RuntimeHarness harness;
        QVERIFY2((harness.isReady()),
                 qPrintable(QStringLiteral("extraction harness must initialize")));
        auto &runtime = harness.runtime();

        {
            ...
            QVERIFY2(
                (validation && validation.get().validatedOnly && validation.get().taskId.isNull() &&
                 ...
                 !harness.pitchStates.isEmpty() && harness.pitchStates.last()->startCount == 0),
                qPrintable(
            ...
            const auto terminal =
                accepted ? runtime.tasks().getTask(base.documentId, accepted.get().taskId)
                         : Automation::AutomationResult<Automation::AutomationTaskSnapshot>(
                               Automation::AutomationError{});
            QVERIFY2(
                (accepted && ran && state->startCount == 1 && terminal &&
                 terminal.get().state == Automation::AutomationTaskState::Succeeded &&
...

        {
            ...
            const auto mergeTerminal =
                mergeAccepted
                    ...
                    : Automation::AutomationResult<Automation::AutomationTaskSnapshot>(
                          Automation::AutomationError{});
            QVERIFY2((mergeAccepted && mergeRan && mergeTerminal &&
...
QTEST_GUILESS_MAIN(TestAutomationAsyncFileDomains)
