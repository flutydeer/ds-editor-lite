...
void NativeDesktopTests::effectiveDurationPolicy() {
    AnimationSettingsReset fixture;
...
void NativeDesktopTests::dialogTitleBarRuntimeUpdate() {
    AnimationSettingsReset fixture;
    auto *theme = ThemeManager::instance();
    theme->setAnimationSettings(true, 2.0);
    ...
    expect(animation->duration() == 600, "dialog deactivation should scale the 300 ms transition");
...
void NativeDesktopTests::progressAndTapTempoLevels() {
    AnimationSettingsReset fixture;
    auto *theme = ThemeManager::instance();
    theme->setAnimationSettings(true, 2.0);
...
void NativeDesktopTests::toolTipImmediateCompletion() {
    AnimationSettingsReset fixture;
    auto *theme = ThemeManager::instance();
    theme->setAnimationSettings(true, 2.0);
    ...
    expect(finishedCount == 1, "an immediate tooltip hide should preserve its completion signal");
    expect(!toolTip.isVisible(), "an immediate tooltip hide should hide the widget");
}

void NativeDesktopTests::toolTipAnchorScreenClamping() {
    AnimationSettingsReset fixture;
