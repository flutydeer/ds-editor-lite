...
void EditorRenderingTests::minimumItemGeometry() {
    ...
    const QRectF expectedMinimumClipRect(3.625, 3.0 + EditorItemGeometry::clipVerticalPadding, 1.0,
    ...
    QVERIFY2(
        (highDpiNoteRect == QRectF(minimumNoteRect.topLeft() * 2.0, minimumNoteRect.size() * 2.0)),
    ...
    QVERIFY2((EditorItemGeometry::adaptiveCornerRadius(
    ...
    EditorRhiGeometry::appendRoundedRect(minimumRoundedClip, minimumClipRect,
                                         EditorItemGeometry::adaptiveCornerRadius(
                                             minimumClipRect, EditorItemGeometry::clipCornerRadius),
                                         QColor(255, 255, 255));
    ...
    const auto legacyClipRect = EditorItemGeometry::clipPaintRect(QRectF(14.0, 4.0, 0.25, 72.0));
    ...
    EditorRhiGeometry::appendRoundedRect(minimumRoundedNote, minimumNoteRect,
                                         EditorItemGeometry::adaptiveCornerRadius(
                                             minimumNoteRect, EditorItemGeometry::noteCornerRadius),
                                         Qt::white);
    QVERIFY2(
...
void EditorRenderingTests::triangleClipping() {
    const QRectF clipRect(0.0, 0.0, 10.0, 10.0);
    ...
    QVERIFY2((accepted.size() == inside.size()), "fully visible triangles must be appended intact");
    ...
    QVERIFY2((clipped.size() == 6), "a triangle crossing one clip edge must become two triangles");
...
void EditorRenderingTests::roundedStrokeClipping() {
    const QRectF clipRect(0.0, 0.0, 10.0, 10.0);
    QVector<EditorRhiSolidVertex> roundedStroke;
    EditorRhiGeometry::appendRoundedRectStroke(roundedStroke, QRectF(2.0, -2.0, 6.0, 8.0), 2.0, 1.2,
                                               QColor(255, 255, 255));
...
void EditorRenderingTests::hairlineDeduplication() {
    const QRectF clipRect(0.0, 0.0, 10.0, 10.0);
    ...
    for (qsizetype index = 0; index + 2 < hairline.size(); index += 3) {
        QVERIFY2(
            (twiceTriangleArea(hairline[index], hairline[index + 1], hairline[index + 2]) > 1e-6),
...
void EditorRenderingTests::shortStrokeEndCaps() {
    const QRectF clipRect(0.0, 0.0, 10.0, 10.0);
    ...
    if (adjustedFlatCap.size() == simplifiedFlatCap.size()) {
        for (qsizetype index = 0; index < adjustedFlatCap.size(); ++index) {
            QVERIFY2((equalVertex(adjustedFlatCap[index], simplifiedFlatCap[index])),
...
void EditorRenderingTests::subpixelOverlayCoverage() {
    const QRectF clipRect(0.0, 0.0, 10.0, 10.0);
...
void EditorRenderingTests::antialiasedCircle() {
    const QRectF clipRect(0.0, 0.0, 10.0, 10.0);
    ...
    for (qsizetype index = 0; index + 2 < circle.size(); index += 3) {
        QVERIFY2((twiceTriangleArea(circle[index], circle[index + 1], circle[index + 2]) > 1e-6),
