...
void EditorInteractionTests::canonicalNoteOrder() {
    {
        SingingClip clip;
        ...
        QVERIFY2(
            (clip.notes().toList() == (QList<Note *>{first, splitContinuation, sameStartHighFirst,
                                                     sameStartHighSecond, sameStartLow, later})),
            "model notes must use start, descending pitch, and id as their canonical order");
    }
}

void EditorInteractionTests::drawAndResizeGeometry() {
    ...
    QVERIFY2((NoteEditUtils::snapLocalDown(310.0, clipStart, quantize, timeline) == 140 &&
...
void EditorInteractionTests::lyricVisibility() {
    ...
    const auto clippedTextRect = wideLayout.textRect.adjusted(fullLyricWidth * 0.5, 0.0, 0.0, 0.0);
    QVERIFY2(
        (NoteLyricPresentation::isElidedInRect(wideLayout, longLyric, lyricFont, clippedTextRect)),
    ...
    QVERIFY2(
        (narrowLayout.isVisible() && narrowLayout.elided && narrowLayout.displayText != longLyric),
...
void EditorInteractionTests::orderedSelection() {
    ...
    QVERIFY2(
        (result.selection == QList<int>{20} && result.targetSelected && selection.anchorId() == 20),
...
void EditorInteractionTests::clickAndDragSelection() {
    using EditorSelectionUtils::OrderedSelectionModel;
    const QList<int> orderedNotes{10, 20, 30, 40, 50};
...
void EditorInteractionTests::contextMenuSelection() {
