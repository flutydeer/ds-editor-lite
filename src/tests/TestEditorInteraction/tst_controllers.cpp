...
void EditorInteractionTests::noView() {
    ...
    expect(!controller->centerTrackPanelAt(1, 2), "track centering without a bound view must fail");
    expect(!controller->setTrackPanelScale(1, 1), "track scaling without a bound view must fail");
...
void EditorInteractionTests::commandCapabilities() {
    ...
    const auto supportsNoCommands = [](const EditorViewGlobal::PianoRollEditMode mode) {
        return !EditorInteraction::supportsCommand(Target::PianoRoll, Command::Cut, mode) &&
               ...
               !EditorInteraction::supportsCommand(Target::PianoRoll, Command::SelectAll, mode) &&
...
void EditorInteractionTests::modeAwareCommandRouting() {
    ...
    const auto commandConnection =
        QObject::connect(controller, &EditorViewController::editCommandRequested,
                         [&commandCount, &requestedCommand](
                             EditorInteraction::Target, const EditorInteraction::Command command) {
                             ++commandCount;
...
void EditorInteractionTests::forwardingAndSnapshots() {
    ...
    expect(view.state.trackPanel.centerTick == 960 && view.state.trackPanel.centerTrackIndex == 1.5,
    ...
    expect(view.state.pianoRoll.centerTick == 1440 && view.state.pianoRoll.centerKeyIndex == 72.5,
...
void EditorInteractionTests::scrollRequiredExecutesOnSecondRequest() {
    ...
    expect(view.revealFocusCount == 1 && view.finalizeFocusCount == 1 &&
               view.clearFocusPreviewCount == 1,
...
void EditorInteractionTests::historyChangeInvalidatesPending() {
    ...
    undoRedoController->requestUndo();
    recordAction(value);
    expect(view.clearFocusPreviewCount == 1,
           "recording a new entry must invalidate pending navigation");
    view.nextFocusVisibility = HistoryFocusVisibility::Visible;
    undoRedoController->requestUndo();
    expect(value == 1, "the new stack top must execute instead of the previously pending entry");
}
