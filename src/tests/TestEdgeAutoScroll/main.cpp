...
class EdgeAutoScrollTests final : public QObject {
    Q_OBJECT
...
private slots:
    ...
    void outsideHotZone() {
        ...
        const auto bothAxes = Qt::Horizontal | Qt::Vertical;
        {
            ...
            QVERIFY2((fuzzyEqual(vEdgeIn.x(), 0)),
                     "inner hot zone boundary must produce zero speed");
        }
    }
    ...
    void edgeDirections() {
        ...
        const auto bothAxes = Qt::Horizontal | Qt::Vertical;
        {
            ...
            QVERIFY2((vLeft.x() < 0 && fuzzyEqual(vLeft.y(), 0)),
                     "left edge scrolls negative x only");
    ...
    void speedSaturation() {
        ...
        const auto bothAxes = Qt::Horizontal | Qt::Vertical;
        {
            ...
            QVERIFY2((fuzzyEqual(-vAtEdge.x(), cfg.maxSpeedH)),
                     "speed at the edge equals maxSpeedH");
            ...
            const auto vOutsideY =
                EdgeAutoScroller::velocity(QPointF(400, 5000), vp, bothAxes, cfg);
    ...
    void cornerAndDisabledAxes() {
        ...
        const auto bothAxes = Qt::Horizontal | Qt::Vertical;
        {
            ...
            const auto vNone =
                EdgeAutoScroller::velocity(QPointF(5, 5), vp, Qt::Orientations(), cfg);
        ...
        const auto bothAxes = Qt::Horizontal | Qt::Vertical;
        {
            ...
            while (elapsed < 1000.0) {
                for (const double dt : dts) {
                    ...
                    const auto step =
                        scroller.computeStep(QPointF(800, 300), vp, bothAxes, clampedDt);
                    scrolled += step.x();
    ...
    void pressAwareStep() {
        ...
        const auto bothAxes = Qt::Horizontal | Qt::Vertical;
        {
            ...
            scroller.resetAccumulator();
            const QPointF pressTL(20, 30);
            const auto stepCenter =
                scroller.computeStep(QPointF(400, 300), pressTL, vp, bothAxes, 16);
            QVERIFY2((stepCenter.x() == 0 && stepCenter.y() == 0),
                     "computeStep with press position in the zone: no scroll toward center");

            const auto stepRight =
                scroller.computeStep(QPointF(790, 300), pressTL, vp, bothAxes, 16);
...
QTEST_GUILESS_MAIN(EdgeAutoScrollTests)
#include "main.moc"
