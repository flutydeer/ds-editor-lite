...
private slots:
    ...
    void validColorsAndSubstitution() {
        ...
        QVERIFY2((styleSheet.has_value()),
                 qPrintable(QStringLiteral("valid placeholders should resolve") +
                            QStringLiteral(": ") + error));
    ...
    void invalidDefinitions() {
        ...
        for (const auto &testCase : cases) {
            ...
            QVERIFY2(
                (!colors && error.contains(testCase.expectedError)),
    ...
    void invalidPlaceholders() {
        ...
        QVERIFY2((colors.has_value()),
                 qPrintable(QStringLiteral("placeholder fixture should parse") +
                            QStringLiteral(": ") + error));
        ...
        QVERIFY2((!result && error.contains(QStringLiteral("Malformed"))),
                 qPrintable(QStringLiteral("malformed placeholder should fail") +
                            QStringLiteral(": ") + error));
    }
    ...
    void externalThemeRoot() {
        ...
        QVERIFY2((writeFile(QDir(themeDir).filePath("base.qss"),
        ...
        QVERIFY2((writeFile(QDir(themeDir).filePath("lyric.qss"),
                            "QWidget { color: ${surface.window}; }")),
    ...
    void bundledThemeLoadingAndFallback() {
        ...
        QVERIFY2((bundled.has_value()),
                 qPrintable(QStringLiteral("bundled theme should load") + QStringLiteral(": ") +
                            ThemeLoader::lastError()));
        const auto bundledLight = ThemeLoader::load(QStringLiteral("lite-light"));
        QVERIFY2((bundledLight.has_value()),
                 qPrintable(QStringLiteral("bundled light theme should load") +
                            QStringLiteral(": ") + ThemeLoader::lastError()));
        ...
        QVERIFY2((bundledAfterFailure.has_value()),
                 qPrintable(QStringLiteral(
                                "bundled theme should remain available after override failure") +
                            QStringLiteral(": ") + ThemeLoader::lastError()));
    }
