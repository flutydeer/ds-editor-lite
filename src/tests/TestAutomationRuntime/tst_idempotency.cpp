...
void AutomationRuntimeTests::serialReplayAndExplicitOptIn() {
    ...
    QVERIFY2((isConflict(changedInput) && isConflict(changedOperation)),
...
void AutomationRuntimeTests::completedReplayPrecedesWorkflowBusyAdmission() {
    AutomationTestSupport::TestRuntime fixture;
    auto &runtime = fixture.runtime();
    auto context = commandContext(runtime, QStringLiteral("d0d00000-0000-4000-8000-000000000008"));
    ...
    const auto blocked = runtime.dispatcher().dispatchIdempotentDocumentCommand(
        Automation::OperationIds::tracks::insert, newContext, QByteArrayLiteral("alpha"), handler);
    runtime.setDocumentBusy(context.expected.documentId, false);

    QVERIFY2(
        (first && replay && replay.get() == first.get() && executions == 1),
        qPrintable(QStringLiteral("completed replay must remain available while workflow busy")));
    QVERIFY2((!conflict &&
    ...
    QVERIFY2(
        ...
        qPrintable(QStringLiteral("workflow busy must still reject a new idempotent mutation")));
}

void AutomationRuntimeTests::concurrentReplayExecutesOnce() {
    ...
    QVERIFY2(
        (sameResult && executions.load(std::memory_order_relaxed) == 1 &&
         runtime.documentVersion().revision == 1),
        qPrintable(QStringLiteral("concurrent opt-in replays must serialize to one execution")));
}

void AutomationRuntimeTests::unsuccessfulAttemptsDoNotClaimKeys() {
    AutomationTestSupport::TestRuntime fixture;
    auto &runtime = fixture.runtime();
    int executions = 0;
    const auto handler = countedHandler(executions);
...
void AutomationRuntimeTests::documentsAndGenerationsHaveIndependentKeySpaces() {
    ...
    QVERIFY2((firstResult && secondResult && reused && executions == 3 && first.revision() == 1 &&
              second.revision() == 1),
             qPrintable(QStringLiteral("document and generation boundaries must isolate keys")));
}

void AutomationRuntimeTests::facadeWiringUsesOptInOnly() {
    AutomationTestSupport::TestRuntime fixture;
    auto &runtime = fixture.runtime();
    ...
    const auto keyedRename = runtime.project().renameTrack(
        commandContext(runtime, QStringLiteral("d0d00000-0000-4000-8000-000000000007")), trackId,
        QStringLiteral("Renamed"));
    const auto rename =
        runtime.project().renameTrack(commandContext(runtime), trackId, QStringLiteral("Renamed"));
...
void AutomationRuntimeTests::retentionIsBounded() {
    Automation::IdempotencyStore store;
    const auto operationId = Automation::OperationIds::tracks::insert;
    for (qsizetype index = 0; index <= Automation::IdempotencyStore::MaximumRetainedKeys; ++index) {
    ...
    QVERIFY2(
        (store.size() == Automation::IdempotencyStore::MaximumRetainedKeys && oldest &&
         !oldest.get() && newest && newest.get() &&
         *newest.get() == Automation::IdempotencyStore::MaximumRetainedKeys),
        qPrintable(QStringLiteral("idempotency retention must evict the oldest completed key")));
}
