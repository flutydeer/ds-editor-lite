...
void AutomationRuntimeTests::taskManagerStateBoundaries() {
    qInfo("task manager state boundaries and stable terminal state");
    ...
    QVERIFY2((task.operationId == Automation::OperationIds::extract::pitch::start &&
    ...
    QVERIFY2((!tasks.succeed(task.taskId, successfulMutation(base))),
    ...
    QVERIFY2((commitAfterCancel && !commitAfterCancel.get() && canceled &&
    ...
    QVERIFY2((cancelAfterTerminal && cancelAfterTerminal.get() == canceled.get() &&
              cancelCallbackCount == 1),
    ...
    QVERIFY2((tasks.markRunning(committingTask.taskId)), "second task must enter Running");
    ...
    QVERIFY2(
        (committing && committing.get() && !lateCancel &&
         lateCancel.getError().code == Automation::AutomationErrorCode::OperationNotCancelable),
        "Committing is the non-cancelable boundary");
    const auto mutation = successfulMutation(base);
    QVERIFY2((tasks.succeed(committingTask.taskId, mutation)),
    ...
    QVERIFY2((!unknownGet && !unknownCancel && !unknownCommit &&
...
void AutomationRuntimeTests::applicationTaskScope() {
    qInfo("application tasks remain independent from document generations");
    ...
    QVERIFY2((canceledTask.scope == Automation::AutomationTaskScope::Application &&
    ...
    QVERIFY2((!tasks.get(document.documentId, canceledTask.taskId) &&
    ...
    QVERIFY2((firstCancel && repeatedCancel && canceledCommit && !canceledCommit.get() &&
              cancelCount == 1 &&
    ...
    QVERIFY2((retainedTasks.markRunning(active.taskId)),
    ...
    for (qsizetype index = 0;
        ...
        QVERIFY2((enteredCommit && enteredCommit.get() &&
    ...
    QVERIFY2((retainedTasks.listApplication().size() ==
...
void AutomationRuntimeTests::documentTaskRetention() {
    qInfo("document terminal task history remains bounded per generation");
    ...
    QVERIFY2((tasks.markRunning(active.taskId)),
    ...
    QVERIFY2((otherCommit && otherCommit.get() &&
              tasks.succeed(other.taskId, {.previous = otherDocument, .current = otherDocument})),
    ...
    for (qsizetype index = 0;
        ...
        QVERIFY2((enteredCommit && enteredCommit.get() &&
                  tasks.succeed(terminal.taskId, {.previous = document, .current = document})),
                 "document history fixture tasks must reach a terminal state");
    }
    QVERIFY2((tasks.list(document.documentId).size() ==
...
void AutomationRuntimeTests::cancelVersusCommitStress() {
    qInfo("cancel versus commit-point barrier stress");
    ...
    for (int iteration = 0; iteration < iterations; ++iteration) {
        ...
        const auto task =
            tasks.createTask(Automation::OperationIds::extract::pitch::start, base, std::nullopt,
                             [&cancelCallbackCount] { cancelCallbackCount.fetch_add(1); });
        QVERIFY2((tasks.markRunning(task.taskId)),
        ...
        if (commitOutcome.returned && commitOutcome.mayCommit) {
            ...
            QVERIFY2(
                (!cancelOutcome.succeeded &&
                 cancelOutcome.error == Automation::AutomationErrorCode::OperationNotCancelable &&
            ...
            ++cancelWins;
            QVERIFY2((commitOutcome.returned && !commitOutcome.mayCommit &&
                      cancelOutcome.succeeded &&
        ...
        QVERIFY2(
            (!tasks.succeed(task.taskId, successfulMutation(base)) && !tasks.cancel(task.taskId)),
            "the terminal state must be immutable after every stress iteration");
    }
    QVERIFY2((cancelWins + commitWins == iterations),
             "every stress iteration must select exactly one race winner");
}

void AutomationRuntimeTests::duplicateCompletionStress() {
    qInfo("duplicate completion callback barrier stress");
    ...
    for (int iteration = 0; iteration < iterations; ++iteration) {
        ...
        const auto task = tasks.createTask(Automation::OperationIds::extract::pitch::start, base);
        QVERIFY2((tasks.markRunning(task.taskId)), "duplicate-callback task must enter Running");
        ...
        std::thread duplicate(complete);
        barrier.arrive_and_wait();
        first.join();
        duplicate.join();

        const auto terminal = tasks.get(base.documentId, task.taskId);
        QVERIFY2((commitCount.load() == 1 && terminalCount.load() == 1 && terminal &&
                  terminal.get().state == Automation::AutomationTaskState::Succeeded),
                 "racing duplicate callbacks must commit and terminalize at most once");
        QVERIFY2(
            (!tasks.fail(task.taskId, Automation::AutomationError{}) && !tasks.cancel(task.taskId)),
            "a duplicate callback cannot replace the successful terminal result");
    }
}

void AutomationRuntimeTests::sessionGenerationContract() {
    qInfo("DocumentSession generation rotation and old TaskId contract");
    ...
    const auto task = tasks.createTask(Automation::OperationIds::extract::pitch::start, oldVersion,
    ...
    QVERIFY2((newVersion.documentId != oldVersion.documentId && newVersion.revision == 0),
    ...
    QVERIFY2((!crossGenerationLookup && crossGenerationLookup.getError().code ==
    ...
    QVERIFY2((cancelCallbacks == 1 && unsuccessfulCallbacks == 1 && terminalCallbacks == 1 &&
              removedSnapshot &&
    ...
    tasks.setTerminalCallback(replacementTask.taskId, [&](const auto &) { ++terminalCallbacks; });
    const auto finalVersion = session.replaceGeneration({}, {});
    tasks.replaceDocumentGeneration(newVersion.documentId, finalVersion);
    QVERIFY2((cancelCallbacks == 2 && unsuccessfulCallbacks == 2 && terminalCallbacks == 2 &&
              tasks.size() == 0),
             "replacing a generation must terminalize every non-preserved task exactly once");
}

void AutomationRuntimeTests::controlledDispatcherCommitBoundary() {
    qInfo("dispatcher final revision recheck after validation barrier");
    {
        ...
        QVERIFY2((runtime.automationTasks().markRunning(task.taskId)),
        ...
        QVERIFY2((validation && validation.get().validatedOnly),
        ...
        QVERIFY2((scheduler.runNext() && interveningEdit),
        ...
        QVERIFY2((committing && committing.get()),
        ...
        QVERIFY2((!lateCancel && lateCancel.getError().code ==
        ...
        QVERIFY2((runtime.automationTasks().fail(task.taskId, std::move(taskError))),
        ...
        QVERIFY2((terminal && terminal.get().state == Automation::AutomationTaskState::Failed &&
                  terminal.get().error &&
                  terminal.get().error->code == Automation::AutomationErrorCode::RevisionConflict &&
        ...
        QVERIFY2((phaseOrder == expectedPhaseOrder),
                 "the controlled phases must prove the final revision recheck order");
    }

    qInfo("non-cancelable commit point and one dispatcher commit");
    {
        AutomationTestSupport::TestRuntime fixture;
        auto &runtime = fixture.runtime();
        const auto base = runtime.documentVersion();
        ...
        QVERIFY2((terminalized && commitAttempts == 1 &&
                  runtime.documentVersion().revision == base.revision + 1),
                 "the real dispatcher mutation must commit exactly once");
        QVERIFY2((!runtime.automationTasks().succeed(task.taskId, committed.get()) &&
        ...
        const auto stableCancel = runtime.tasks().cancelTask(
            {.expected = runtime.documentVersion(), .source = Automation::InvocationSource::Test},
            task.taskId);
        QVERIFY2((stableCancel &&
    ...
    qInfo("public task cancellation while document workflow is busy");
    {
        AutomationTestSupport::TestRuntime fixture;
        auto &runtime = fixture.runtime();
        const auto base = runtime.documentVersion();
        ...
        QVERIFY2((running && busy && canceled &&
...
void AutomationRuntimeTests::committingGenerationReplacement() {
    qInfo("generation replacement supersedes a task at the commit point");
    ...
    const auto replacement =
        runtime.documents().commitNewDocument(replaceContext, RaceFixture::emptyDocumentDraft());
    const auto replacementVersion = runtime.documentVersion();
    QVERIFY2((replacement && replacementVersion.documentId != base.documentId &&
    ...
    QVERIFY2((cancelCallbackCount == 0 &&
    ...
    QVERIFY2((!oldLookup &&
              ...
              !newCancel && newCancel.getError().code == Automation::AutomationErrorCode::NotFound),
             "old and unknown TaskId errors must stay stable for both query and cancel after "
             "replacement");
}

void AutomationRuntimeTests::queuedAndRunningCancellation() {
    qInfo("queued cancel before fake scheduler release");
    {
        RaceFixture fixture;
        QVERIFY2((fixture.isReady()), "race fixture must create typed audio and singing clips");
        ObserverLog observer;
        const auto accepted = fixture.startPitch(observer.observer());
        QVERIFY2((accepted && fixture.scheduler().pendingCount() == 1 &&
        ...
        QVERIFY2((fixture.scheduler().runNext()),
        ...
        QVERIFY2((terminal && terminal.get().state == Automation::AutomationTaskState::Canceled &&
                  state->startCount == 0 &&
    ...
    qInfo("running cancel before backend completion");
    {
        RaceFixture fixture;
        QVERIFY2((fixture.isReady()), "race fixture must initialize");
        ObserverLog observer;
        const auto base = fixture.runtime().documentVersion();
        const auto accepted = fixture.startPitch(observer.observer());
        QVERIFY2((accepted && fixture.scheduler().runNext()),
        ...
        QVERIFY2((running && running.get().state == Automation::AutomationTaskState::Running &&
                  state->startCount == 1),
        ...
        const auto terminal =
            fixture.runtime().tasks().getTask(base.documentId, accepted.get().taskId);
        QVERIFY2((cancel && repeatedCancel && terminal &&
                  terminal.get().state == Automation::AutomationTaskState::Canceled &&
...
void AutomationRuntimeTests::completionPermutations() {
    qInfo("successful completion before cancel and duplicate callback");
    {
        RaceFixture fixture;
        QVERIFY2((fixture.isReady()), "race fixture must initialize");
        ObserverLog observer;
        ...
        QVERIFY2((firstTerminal && stableTerminal && lateCancel &&
    ...
    qInfo("cancel plus revision advance before completion");
    {
        RaceFixture fixture;
        QVERIFY2((fixture.isReady()), "race fixture must initialize");
        const auto base = fixture.runtime().documentVersion();
...
void AutomationRuntimeTests::revisionAndObjectDeletion() {
    qInfo("revision advance before extraction completion");
    {
        RaceFixture fixture;
        QVERIFY2((fixture.isReady()), "race fixture must initialize");
        ObserverLog observer;
        const auto base = fixture.runtime().documentVersion();
        const auto accepted = fixture.startPitch(observer.observer());
        QVERIFY2((accepted && fixture.scheduler().runNext()), "revision race must reach Running");
        if (!accepted || fixture.pitchStates().isEmpty())
            return;
    ...
    qInfo("target deletion before extraction completion");
    {
        RaceFixture fixture;
        QVERIFY2((fixture.isReady()), "race fixture must initialize");
        const auto base = fixture.runtime().documentVersion();
        ...
        if (!accepted || fixture.pitchStates().isEmpty())
            return;
        const auto removed =
            fixture.runtime().project().removeClips(fixture.context(), {fixture.singingClipId()});
        fixture.pitchStates().last()->complete(RaceFixture::successfulPitch());
        const auto terminal =
            fixture.runtime().tasks().getTask(base.documentId, accepted.get().taskId);
        QVERIFY2((removed && removed.get().changed && terminal &&
                  terminal.get().state == Automation::AutomationTaskState::Failed &&
                  terminal.get().error &&
                  terminal.get().error->code == Automation::AutomationErrorCode::RevisionConflict &&
...
void AutomationRuntimeTests::newAndOpenGenerationReplacement() {
    qInfo("new generation replaces a queued extraction");
    {
        RaceFixture fixture;
        QVERIFY2((fixture.isReady()), "race fixture must initialize");
        ObserverLog observer;
        ...
        QVERIFY2((fixture.scheduler().runNext() && state->startCount == 0),
    ...
    qInfo("open generation replaces a running extraction");
    {
        RaceFixture fixture;
        QVERIFY2((fixture.isReady()), "race fixture must initialize");
        ObserverLog observer;
        ...
        QVERIFY2((replacement && state->startCount == 1 && state->cancelCount == 1 &&
        ...
        const auto oldLookup =
            fixture.runtime().tasks().getTask(oldVersion.documentId, accepted.get().taskId);
        const auto newLookup =
            fixture.runtime().tasks().getTask(replacementVersion.documentId, accepted.get().taskId);
        QVERIFY2((fixture.runtime().documentVersion() == replacementVersion && !oldLookup &&
                  oldLookup.getError().code == Automation::AutomationErrorCode::DocumentChanged &&
                  !newLookup &&
                  newLookup.getError().code == Automation::AutomationErrorCode::NotFound &&
                  observer.snapshots.isEmpty()),
