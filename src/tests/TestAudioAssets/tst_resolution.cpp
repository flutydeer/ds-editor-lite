...
void AudioAssetsTests::hashSnapshot() {
    ...
    QVERIFY2((snapshotFile.open(QIODevice::ReadOnly)), "the completed snapshot must be readable");
...
void AudioAssetsTests::hitRelative() {
    QTemporaryDir fixtures;
    QVERIFY(fixtures.isValid());
    const auto root = fixtures.path();
...
void AudioAssetsTests::hitSibling() {
    QTemporaryDir fixtures;
    QVERIFY(fixtures.isValid());
    const auto root = fixtures.path();
...
void AudioAssetsTests::hitUnconfirmed() {
    QTemporaryDir fixtures;
    QVERIFY(fixtures.isValid());
    const auto root = fixtures.path();
...
void AudioAssetsTests::hashMismatch() {
    QTemporaryDir fixtures;
    QVERIFY(fixtures.isValid());
    const auto root = fixtures.path();
...
void AudioAssetsTests::currentDirectoryDecoy() {
    QTemporaryDir fixtures;
    QVERIFY(fixtures.isValid());
    const auto root = fixtures.path();
...
void AudioAssetsTests::directoryCandidateRejected() {
    QTemporaryDir fixtures;
    QVERIFY(fixtures.isValid());
    const auto root = fixtures.path();
...
void AudioAssetsTests::derivedAudioWritebacks() {
    ...
    QVERIFY2((validationContext && validationContext.get().expected == editedVersion &&
    ...
    if (staleContext) {
        ...
        const auto staleStatus =
            runtime.project().setAudioClipPathStatus(staleContext.get(), fixture.audioClipId(),
                                                     initialAsset, AudioClip::PathStatus::Missing);
        QVERIFY2((isStaleAssetError(staleResolved,
                                    Automation::OperationIds::audio_clips::apply_resolved_path) &&
    ...
    QVERIFY2((decoded && decoded.get().changed && decoded.get().previous == editedVersion &&
              decoded.get().current == editedVersion && runtime.documentVersion() == editedVersion),
    ...
    QVERIFY2(
        (hashResult && hashResult.get().changed && hashResult.get().previous == editedVersion &&
         hashResult.get().current == editedVersion && runtime.documentVersion() == editedVersion),
        "set_hash must succeed without advancing revision");

    const auto hashedClip = audioClipSnapshot(runtime, fixture.audioClipId());
    QVERIFY2((static_cast<bool>(hashedClip)), "the hashed audio snapshot must remain queryable");
    if (!hashedClip)
        return;
    ...
    const auto status =
        statusContext
            ...
            : Automation::AutomationResult<Automation::MutationResult>(statusContext.getError());
    QVERIFY2((status && status.get().changed && status.get().previous == editedVersion &&
              status.get().current == editedVersion && runtime.documentVersion() == editedVersion),
    ...
    const auto replacement =
        runtime.documents().commitNewDocument(fixture.context(), RuntimeFixture::emptyDocument());
    ...
    QVERIFY2(
        (!rejectedGeneration &&
         rejectedGeneration.getError().code == Automation::AutomationErrorCode::DocumentChanged &&
...
void AudioAssetsTests::samePathSourceReplacement() {
    ...
    QVERIFY2((replacementClip && replacementClip->sourceGeneration() > taskAsset.sourceGeneration),
    ...
    QVERIFY2((isStaleAssetError(strict, Automation::OperationIds::audio_clips::apply_decode_cache)),
    ...
    const auto staleCache =
        rebasedContext
            ? runtime.project().applyAudioDecodeCache(rebasedContext.get(), fixture.audioClipId(),
                                                      taskAsset, staleInfo)
            : Automation::AutomationResult<Automation::MutationResult>(rebasedContext.getError());
    const auto staleHash =
        rebasedContext
            ? runtime.project().setAudioClipHash(rebasedContext.get(), fixture.audioClipId(),
                                                 taskAsset,
                                                 sha512(QByteArray("stale-audio-payload")))
            : Automation::AutomationResult<Automation::MutationResult>(rebasedContext.getError());
    const auto staleStatus =
        rebasedContext
            ? runtime.project().setAudioClipPathStatus(rebasedContext.get(), fixture.audioClipId(),
                                                       taskAsset, AudioClip::PathStatus::Missing)
            : Automation::AutomationResult<Automation::MutationResult>(rebasedContext.getError());
    QVERIFY2(
        (isStaleAssetError(staleCache, Automation::OperationIds::audio_clips::apply_decode_cache) &&
         isStaleAssetError(staleHash, Automation::OperationIds::audio_clips::set_hash) &&
         isStaleAssetError(staleStatus, Automation::OperationIds::audio_clips::set_path_status)),
...
void AudioAssetsTests::resolvedPathValidation() {
    ...
    const auto directory =
        runtime.project().applyResolvedAudioPath(fixture.context(), fixture.audioClipId(), asset,
                                                 targets.path(), AudioClip::PathStatus::Normal);
...
void AudioAssetsTests::samePathResolutionNotification() {
    ...
    const auto sourcePath = QDir(sourceFiles.path()).filePath(QStringLiteral("same-source.wav"));
    ...
    QVERIFY2((missing && resolved && resolved.get().changed && sourceChangeCount == 1 &&
              clip->path() == sourcePath && clip->pathStatus() == AudioClip::PathStatus::Normal &&
    ...
    QObject::connect(clip, &AudioClip::sourceChanged, clip,
                     [clip, &statusAtSourceChange] { statusAtSourceChange = clip->pathStatus(); });
    const auto unconfirmed = runtime.project().applyResolvedAudioPath(
        fixture.context(), fixture.audioClipId(), Automation::audioAssetSnapshotDto(*clip),
        sourcePath, AudioClip::PathStatus::Unconfirmed);
    QVERIFY2((missingAgain && unconfirmed && unconfirmed.get().changed && sourceChangeCount == 1 &&
    ...
    QVERIFY2((decoded && decoded.get().changed &&
              clip->pathStatus() == AudioClip::PathStatus::Unconfirmed &&
              sameAudioInfo(clip->audioInfo(), decodedInfo) && runtime.documentVersion() == base),
             "successful decoding must not implicitly confirm a name-only audio match");
}

void AudioAssetsTests::samePathRelinkNotification() {
...
void AudioAssetsTests::resolvedPathSourceFailureWins() {
    ...
    const auto resolved =
        runtime.project().applyResolvedAudioPath(fixture.context(), fixture.audioClipId(), source,
...
void AudioAssetsTests::relinkHistoryNotifications() {
    ...
    const auto relocate =
        runtime.project().relocateAudioClip(fixture.context(), fixture.audioClipId(),
                                            replacementPath, replacementInfo, replacementFormat);
...
void AudioAssetsTests::resolveDecodeTaskProtocol() {
    ...
    QObject::connect(clip, &AudioClip::sourceChanged, clip, [&] {
        ...
        decodeRecord = runtime.automationTasks().createTask(
            Automation::OperationIds::audio_clips::apply_decode_cache, runtime.documentVersion(),
            Automation::ObjectRef{Automation::ObjectKind::Clip, fixture.audioClipId().value()});
    ...
    const auto resolveCommitting = runtime.automationTasks().beginCommitting(resolveRecord.taskId);
    ...
    QVERIFY2((resolveSucceeded && sourceChangeCount == 1 && decodeAsset && decodeRecord &&
              decodeRunning && decodeAsset->sourceGeneration == resolveAsset.sourceGeneration + 1 &&
    ...
    const auto decodeCommitting = runtime.automationTasks().beginCommitting(decodeRecord->taskId);
    ...
    QVERIFY2((decodeSucceeded && resolveTerminal && decodeTerminal &&
              ...
              sameAudioInfo(clip->audioInfo(), decodedInfo) && runtime.documentVersion() == base &&
              historyAfter && sameHistoryState(historyBefore.get(), historyAfter.get())),
             "resolve and decode tasks must succeed without revision or history growth");
}

void AudioAssetsTests::deletedAudioTargetTerminalState() {
    ...
    const auto removed = runtime.project().removeClips(fixture.context(), {fixture.audioClipId()});
    ...
    QVERIFY2(
        (writebackError && writebackError->code == Automation::AutomationErrorCode::NotFound &&
         writebackError->operationId == Automation::OperationIds::audio_clips::apply_decode_cache &&
...
void AudioAssetsTests::hashDecodeOrdering() {
    ...
    if (firstContext) {
        first = hashFirst ? runtime.project().setAudioClipHash(
                                firstContext.get(), fixture.audioClipId(), taskAsset, hash)
    ...
    if (secondContext) {
        second = hashFirst ? runtime.project().applyAudioDecodeCache(
                                 secondContext.get(), fixture.audioClipId(), taskAsset, audioInfo)
    ...
    QVERIFY2((first && first.get().changed && second && second.get().changed && finalClip &&
              finalClip->audioPathInfo.sha512 == hash &&
              sameAudioInfo(finalClip->audioInfo, audioInfo) &&
              finalClip->audioPathStatus == AudioClip::PathStatus::Normal &&
