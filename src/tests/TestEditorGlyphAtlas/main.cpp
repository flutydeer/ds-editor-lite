...
class EditorGlyphAtlasTests final : public QObject {
    Q_OBJECT
...
private slots:
    ...
    void coverageAndAtlasRows() {
        ...
        QVERIFY2((batches.size() == 1), "two small blocks must share one atlas page");
        if (batches.size() == 1) {
            ...
            QVERIFY2((hasCoverage), "rasterized text must produce atlas coverage");
            QVERIFY2((hasValidPremultipliedCoverage),
                     "subpixel coverage must remain valid premultiplied RGBA data");

            const auto &vertices = batches.front().vertices;
            QVERIFY2((vertices.size() == 12), "both text blocks must emit one quad");
            for (const auto &vertex : vertices) {
                QVERIFY2((vertex.u >= 0.0f && vertex.u <= 1.0f),
                         "horizontal texture coordinates must stay inside the atlas");
                QVERIFY2((vertex.v >= 0.0f && vertex.v <= 1.0f),
                         "vertical texture coordinates must stay inside the atlas");
            }
            if (vertices.size() == 12)
                QVERIFY2((vertices[6].v > vertices[0].v),
    ...
    void fractionalDprAndTextureReplacement() {
        ...
        QVERIFY2((fractionalBatches.size() == 1 && fractionalBatches.front().vertices.size() == 6),
        ...
        if (fractionalBatches.size() == 1 && fractionalBatches.front().vertices.size() == 6) {
            ...
            QVERIFY2((rasterWidth >= expectedWidth && rasterWidth <= expectedWidth + 1.0),
            ...
            QVERIFY2((clearedBatches.size() == 1 &&
                      clearedBatches.front().generation > previousGeneration),
    ...
    void subpixelPhaseCache() {
        ...
        QVERIFY2((phaseBatches.size() == 1 && phaseBatches.front().vertices.size() == 18),
    ...
    void cameraRelativeAlignment() {
        ...
        cameraAtlas.appendText(text, font, QPointF(20.4, 30.2), Qt::white, {}, 1.0,
                               QPointF(10, 20));
        cameraAtlas.appendText(text, font, QPointF(21.4, 31.2), Qt::white, {}, 1.0,
                               QPointF(11, 21));
        QVector<EditorRhiTextureBatch> cameraBatches;
        cameraAtlas.populateTextureBatches(cameraBatches);
        QVERIFY2((cameraBatches.size() == 1 && cameraBatches.front().vertices.size() == 12),
    ...
    void windowRelativeAlignment() {
        ...
        windowAtlas.appendText(text, font, QPointF(20.9, 30.2), Qt::white, {}, 1.0, QPointF(10, 20),
                               QPointF(0, 0));
        QVERIFY2((qFuzzyCompare(windowAtlas.hitRate(), 0.5)),
    ...
    void drawOrderPreservesOcclusion() {
        ...
        const auto firstText =
            orderedAtlas.appendText(text, font, QPointF(0, 0), Qt::white, {}, 1.0);
        ...
        QVERIFY2((commands.size() == 4), "solid and text draws must preserve their append order");
        if (commands.size() == 4) {
            QVERIFY2((commands[0].type == EditorRhiDrawCommand::Type::Solid &&
    ...
    void blendColorsRemainDistinct() {
        ...
        const auto whiteText =
            coloredAtlas.appendText(text, font, QPointF(0, 0), Qt::white, {}, 1.0);
        const auto blackText =
            coloredAtlas.appendText(text, font, QPointF(0, 20), Qt::black, {}, 1.0);
        coloredDrawList.appendTexture(whiteText, 0);
        coloredDrawList.appendTexture(blackText, 0);
        QVERIFY2((coloredDrawList.commands.size() == 2),
                 "adjacent text draws with different blend colors must remain separate");
        if (coloredDrawList.commands.size() == 2) {
            QVERIFY2((coloredDrawList.commands[0].color == Qt::white &&
    ...
    void currentFramePagesAreNotEvicted() {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            QVERIFY2((vertexCount >= previousVertexCount),
...
QTEST_MAIN(EditorGlyphAtlasTests)
#include "main.moc"
