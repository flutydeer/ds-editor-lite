...
void NoteActions::insertNotes(const QList<Note *> &notes, DsSingingClip *clip) {
    for (const auto note : notes)
        addAction(InsertNoteAction::build(note, clip));
}
void NoteActions::removeNotes(const QList<Note *> &notes, DsSingingClip *clip) {
    for (const auto note : notes)
...
void NoteActions::editNotesLength(const QList<Note *> &notes, int delta, DsSingingClip *clip) {
    for (const auto note : notes)
