...
namespace FillLyric {
    ...
    QPainterPath LyricCell::shape() const {
        QPainterPath path;
        path.addRect({syllablePos().x(), syllablePos().y(), syllableWidth(),
    ...
    qreal LyricCell::width() const {
        return std::max(lyricWidth(), syllableWidth()) + m_padding * 2 + m_rectBorder * 2 +
               m_rectPadding * 2;
    }

    qreal LyricCell::height() const {
        return m_padding + m_sRect.height() + m_lsPadding + m_lRect.height() + m_rectBorder * 2 +
               m_rectPadding * 2;
    }
    ...
    qreal LyricCell::margin() const {
        return m_rectPadding;
    }

    void LyricCell::setMargin(const qreal &margin) {
        m_rectPadding = margin;
    }
    ...
    void LyricCell::handleMouseSelection(QGraphicsSceneMouseEvent *event) {
        if (event->button() == Qt::RightButton && scene()->selectedItems().size() > 1 &&
            this->isSelected()) {
            event->accept();
            return;
        }
    ...
    void LyricCell::contextMenuEvent(QGraphicsSceneContextMenuEvent *event) {
        ...
        if (lRect.contains(event->scenePos())) {
            menu->addSeparator();
            menu->addAction(QCoreApplication::translate("LyricCell", "clear cell"), this,
                            [this] { this->clear(); });
            if (m_cells->size() == 1)
                menu->addAction(QCoreApplication::translate("LyricCell", "delete line"), this,
                                [this] { Q_EMIT this->deleteLine(this); });
            else
                menu->addAction(QCoreApplication::translate("LyricCell", "delete cell"), this,
                                [this] { Q_EMIT this->deleteCell(this); });
            menu->addAction(QCoreApplication::translate("LyricCell", "add prev cell"), this,
                            [this] { Q_EMIT this->addPrevCell(this); });
            menu->addAction(QCoreApplication::translate("LyricCell", "add next cell"), this,
                            [this] { Q_EMIT this->addNextCell(this); });
            if (m_cells->indexOf(this) != 0)
                menu->addAction(QCoreApplication::translate("LyricCell", "linebreak"), this,
                                [this] { Q_EMIT this->linebreak(this); });
        }
    ...
    void LyricCell::changePhonicMenu(QMenu *menu) {
        auto *inputAction =
            new QAction(QCoreApplication::translate("LyricCell", "Custom Syllables"), this);
        menu->addAction(inputAction);
        connect(inputAction, &QAction::triggered, this, [this] {
            bool ok;
            const QString syllable = QInputDialog::getText(
                m_view, QCoreApplication::translate("LyricCell", "Custom Syllables"),
                QCoreApplication::translate("LyricCell", "Please input syllables"),
                QLineEdit::Normal, "", &ok);
