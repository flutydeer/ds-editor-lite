...
namespace FillLyric {
    LyricWrapView::LyricWrapView(QString qssPath, QStringList priorityG2pIds,
        ...
        : QGraphicsView(parent), m_qssPath(std::move(qssPath)),
          m_priorityG2pIds(std::move(priorityG2pIds)), m_langToG2pId(std::move(langToG2pId)) {
    ...
    void LyricWrapView::contextMenuEvent(QContextMenuEvent *event) {
        ...
        if (selectedItems.size() > 1) {
            ...
            if (enableMenu && !m_selectedCells.isEmpty()) {
                menu.addAction(QCoreApplication::translate("LyricWrapView", "clear cells"), [this] {
                ...
                if (m_selectedCells.size() > 1) {
                    ...
                    if (this->cellEqualLine(m_selectedCells)) {
                        menu.addAction(QCoreApplication::translate("LyricWrapView", "delete line"),
                                       [this, cellList] { this->removeList(cellList); });
                    } else
                        menu.addAction(QCoreApplication::translate("LyricWrapView", "delete cells"),
        ...
        if (dynamic_cast<HandleItem *>(itemAtPos) && handleItems.size() > 1) {
            ...
            menu.addSeparator();
            menu.addAction(QCoreApplication::translate("LyricWrapView", "delete lines"),
                           [this, selectedSet] {
            ...
            menu.addAction(QCoreApplication::translate("LyricWrapView", "move up"),
                           [this, selectedSet] { this->moveUpLists(QList(selectedSet.values())); });
            menu.addAction(
                QCoreApplication::translate("LyricWrapView", "move down"),
                [this, selectedSet] { this->moveDownLists(QList(selectedSet.values())); });
    ...
    CellList *LyricWrapView::createNewList() {
        ...
        const auto cellList =
            new CellList(0, 0, {}, m_scene, this, &m_cellLists, m_priorityG2pIds, m_langToG2pId);
    ...
    void LyricWrapView::appendList(const QList<LangNote *> &noteList) {
        ...
        const auto cellList =
            new CellList(0, cellBaseY(static_cast<int>(m_cellLists.size())), noteList, m_scene,
                         this, &m_cellLists, m_priorityG2pIds, m_langToG2pId);
        cellList->setFont(this->font());
        cellList->setWidth(width);
        m_cellLists.append(cellList);
        this->connectCellList(cellList);
    ...
    QStringList LyricWrapView::priorityG2pIds() const {
        return m_priorityG2pIds;
    }
