...
namespace FillLyric {
    QList<G2pResult> G2pService::convert(const QList<LangNote> &notes,
        ...
        for (int i = 0; i < notes.size(); i++) {
            ...
            std::string g2pId;
            if (notes[i].g2pId != QStringLiteral("unknown") &&
                notes[i].g2pId != QStringLiteral("slur") && !notes[i].g2pId.isEmpty()) {
                g2pId = notes[i].g2pId.toStdString();
            } else {
                g2pId = langToG2pId.value(language, "g2p-" + language + "-official");
            }
            g2pInputs.push_back(LangCore::G2pInput(taggerRes[i].lyric, g2pId));
        }
        ...
        const int resultCount =
            qMin(static_cast<int>(g2pRes.size()), static_cast<int>(taggerRes.size()));
        for (int i = 0; i < resultCount; i++) {
            ...
            result.g2pId = QString::fromStdString(
                langToG2pId.value(language, "g2p-" + language + "-official"));
    ...
    G2pResult G2pService::convertSingle(const LangNote &note,
                                        const std::vector<std::string> &priorityG2pIds,
                                        const QMap<std::string, std::string> &langToG2pId) {
