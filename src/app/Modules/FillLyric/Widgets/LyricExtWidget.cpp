...
namespace FillLyric {
    LyricExtWidget::LyricExtWidget(int *notesCount, const LyricTabConfig &config,
        ...
        m_wrapView =
            new LyricWrapView(":/theme/lyricwarpview-dark.qss", priorityG2pIdList, m_langToG2pId);
        m_wrapView->setContentsMargins(0, 0, 0, 0);

        m_tableTopLayout = new QHBoxLayout();
        m_tableTopLayout->setContentsMargins(0, 0, 0, 0);
        m_btnFoldLeft = new QPushButton(QCoreApplication::translate("LyricExtWidget", "Fold Left"));

        m_btnInsertText = new QPushButton(QCoreApplication::translate("LyricExtWidget", "Test"));
