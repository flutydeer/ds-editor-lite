...
void DocumentImportController::commitBatch() {
    ...
    for (int preparedIndex = 0; preparedIndex < m_prepared.size(); ++preparedIndex) {
        ...
        if (item.kind == PreparedImportItem::Kind::Audio) {
            ...
            const std::unique_ptr<AudioClip> clip(buildAudioClip(item.audio, startTick, finalTimeline));
            ...
            const auto generatedIt = std::find_if(
                generatedMidis.begin(), generatedMidis.end(), [preparedIndex](const auto &generated) {
