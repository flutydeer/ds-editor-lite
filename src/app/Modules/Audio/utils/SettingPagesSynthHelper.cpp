#include "SettingPagesSynthHelper.h"
...
static struct {
    ...
    QString selectTooltip() const {
        if (enabled) {
            return QRandomGenerator::global()->bounded(0, 2)
                       ? QCoreApplication::translate("Audio::Internal::SettingPageSynthHelper",
                                                     "Why did you play ...?!")
                       : QCoreApplication::translate("Audio::Internal::SettingPageSynthHelper",
...
SettingPageSynthHelper::SettingPageSynthHelper(QObject *parent)
    : QObject(parent), m_adoptedScores(scores) {
}
...
void SettingPageSynthHelper::initialize(
    ...
    connect(amplitudeSlider, &SVS::SeekBar::valueChanged, this, [=](const double value) {
    ...
    connect(amplitudeSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
            [amplitudeSlider, this](const double decibel) {
    ...
    connect(attackSpinBox, QOverload<int>::of(&QSpinBox::valueChanged), this,
            [attackSlider, this](const int value) {
    ...
    connect(decaySpinBox, QOverload<int>::of(&QSpinBox::valueChanged), this,
            [decaySlider, this](const int value) {
    ...
    connect(decayRatioSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
            [decayRatioSlider, this](const double value) {
    ...
    connect(releaseSpinBox, QOverload<int>::of(&QSpinBox::valueChanged), this,
            [decayRatioSlider, releaseSlider, this](const int value) {
                QSignalBlocker o(decayRatioSlider);
    ...
    m_testSynthesizer.setGenerator(
        static_cast<talcs::NoteSynthesizer::Generator>(m_cachedGenerator));
    m_testMixer.setGain(talcs::Decibels::decibelsToGain(m_cachedAmplitude));
    m_testSynthesizer.setAttackTime(
        AudioHelpers::msecToSample(m_cachedAttackMsec, m_testSynthesizer.sampleRate()));
    m_testSynthesizer.setDecayTime(
        AudioHelpers::msecToSample(m_cachedDecayMsec, m_testSynthesizer.sampleRate()));
    m_testSynthesizer.setDecayRatio(m_cachedDecayRatio);
    m_testSynthesizer.setReleaseTime(
        AudioHelpers::msecToSample(m_cachedReleaseMsec, m_testSynthesizer.sampleRate()));

    connect(this, &SettingPageSynthHelper::testFinished, previewButton,
            [=] { previewButton->setToolTip(easterEggHelper.selectTooltip()); });
}

void SettingPageSynthHelper::detectInterval(const qint64 intervalLength) {
...
void SettingPageSynthHelper::toggleTestState(const bool enabled) {
    QMutexLocker locker(&m_mutex);
    if (enabled) {
