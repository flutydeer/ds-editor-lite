...
public:
    explicit EventDiagFilter(QObject *parent = nullptr) : QObject(parent) {
...
private:
    void printStats() {
        ...
        qDebug().noquote().nospace()
            ...
            << "/s | Input=" << qSetFieldWidth(3) << int(inputsPerSec) << qSetFieldWidth(0)
            << "/s (" << QString::number(inputRatio, 'f', 1) << "%)";
