...
class InferencePageWidget : public QWidget {
    Q_OBJECT
public:
    explicit InferencePageWidget(QWidget *parent = nullptr) : QWidget(parent) {
        ...
        for (const auto &device : std::as_const(deviceList)) {
            auto displayText = QStringLiteral("[%1] %2 (%3 GiB)")
                    .arg(device.index)
...
InferencePage::InferencePage(QWidget *parent) : IOptionPage(parent) {
    ...
    m_widget = new InferencePageWidget;
