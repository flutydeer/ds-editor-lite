...
void TimeSignatureActions::setTimeSignatureAt(const TimeSignature &signature, AppModel *model) {
    ...
    const bool exists = std::any_of(oldSignatures.cbegin(), oldSignatures.cend(),
                                    [&](const TimeSignature &existing) {
                                        return existing.barIndex == signature.barIndex;
                                    });
